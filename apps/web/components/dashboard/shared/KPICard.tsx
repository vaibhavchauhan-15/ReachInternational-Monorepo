...
export interface KPICardProps {
  label: string;
  value: number | string;
  icon: LucideIcon | React.ComponentType<{ className?: string; size?: number | string }> | React.ReactNode;
...
const variantStyles = {
  default: {
    ...
    iconBg: "bg-slate-500/10 text-slate-700 dark:text-slate-300 border border-slate-500/15",
  ...
  const content = (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-1.5 sm:gap-2">
        ...
        <div className={`w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-md sm:rounded-lg shrink-0 ${styles.iconBg}`}>
