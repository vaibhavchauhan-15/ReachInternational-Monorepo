...
- **Centralized Data Access Layer & Lean Projection (Phase 1 Performance Program)**: Modularized User DAL (`apps/web/lib/data/users/`) into `user-list.ts`, `user-detail.ts`, `user-shared.ts`, and `index.ts`, mirroring the established `machine-list.ts` pattern. Preserves `apps/web/lib/queries/users.ts` as a 100% backward-compatible facade. Replaced unbounded `SELECT *` with lean 16-column projection (`USER_LIST_COLUMNS`), stripping heavy sensitive fields (`aadhaar_number`, `license_number`, `address`) from list loads. Replaced eager relational SQL joins on `users` and `working_locations` with sub-millisecond in-memory hydration from Next.js cached master queries (`getActiveSupervisorsCached` and `getActiveWorkingLocationsCached`). Modals (`UserDetailSheet`, `UserEditModal`) fetch full 21-column records on-demand via `getUserDetailAction`.
