-- ============================================================================
-- Migration 107: Enhance Attendance Monthly Summary Filters
-- 0 new tables. Updates get_attendance_monthly_summary RPC.
-- Supports advanced filters: overtime, state, search by city/state, sort_by,
-- and extended attendance status (has_absences, perfect).
-- ============================================================================

-- Drop old 7-parameter version so signature is unique and backwards-compatible with named calls
DROP FUNCTION IF EXISTS public.get_attendance_monthly_summary(int, int, text, text, text, int, int);
  ...
  WITH employee_attendance AS (
    ...
    WHERE u.role = 'operator'
      ...
      AND (p_search IS NULL OR p_search = '' OR
           ...
           u.city ILIKE '%' || p_search || '%' OR
           u.state ILIKE '%' || p_search || '%')
  ),
  filtered AS (
    SELECT *
