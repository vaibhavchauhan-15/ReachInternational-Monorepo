...
- **Assignments & Personnel (24h Multi-Shift Fleet Coverage)**:
  ...
  - `operator_ids UUID[]`: Authoritative array of active assigned operators. Trigger `sync_machine_personnel_arrays` sets `current_operator_id := operator_ids[1]` (or NULL when empty) without auto-prepending stale operators. Trigger `sync_machine_operator_assignments_on_machine_update` (Migration 104) automatically deactivates entries in `operator_machine_assignments` when an operator is removed from `operator_ids`. Read models and UI never fall back to default/single fields when `operator_ids` is empty (`[]`).
...
- **Multi-User Selection & Instant Assignment Auto-Sync**: `MultiUserSelect` supports selecting multiple supervisors AND operators with searchable chips, shift timing metadata, removable individual chips, and a "Deselect All" action. Saved via `updateMachineSupervisorsAction` and `updateMachineOperatorsAction` server actions which directly return fresh assignment records. The UI updates immediately without full-page reloads (`router.refresh()` / `window.location.reload()`), and verifies in the background via `getMachinePersonnelFreshAction`.
