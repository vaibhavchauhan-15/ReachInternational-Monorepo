...
## Supported System Roles (13 Roles)
1. `super_admin` — Super Admin (Unrestricted global platform & multi-branch authority; full access across all 19 modules; immutable audit log view & export)
2. `admin` — System Administrator (User onboarding & machine master administration)
3. `branch_manager` — Branch Manager (Assigned branch fleet, inventory & staff control; no hard delete, no salary visibility, no direct stock execution, no user role mutation)
...
11. `rental_manager` — Rental Manager (Machinery rental contracts & dispatches)
12. `sales_executive` — Sales Executive (Leads, quotations & sales pipeline)
13. `finance_manager` — Finance Manager (Invoices, payments & financial reports)
...
- `updateUserRole(userId, role)`: Changes user permission tier across all 13 system roles (Super Admin can assign any role including Super Admin; Admin cannot modify Super Admin roles).
