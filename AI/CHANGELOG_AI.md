- **Bug Fix: Database Migration 051 — Fix Undefined Object `type "public.user_role" does not exist` in `handle_new_user()` Trigger (`supabase/migrations/051_include_lunch_in_shift_and_add_user_shift_times.sql`) (2026-09-08)**:
...
- **Bug Fix & Hardening: Complete Resolution of All Operator QA Bugs (BUG-OP-01 to BUG-OP-08) across Database, Server Actions, Web & Mobile (`packages/validation/src/hourMeter.ts`, `apps/web/app/actions/operators.ts`, `apps/web/components/dashboard/OperatorDashboard.tsx`, `apps/web/components/ui/MachineSelect.tsx`, `apps/mobile/components/work/MeterLogModal.tsx`, `supabase/migrations/052_fix_operator_qa_bugs_and_security_hardening.sql`, `supabase/tests/test_operator_complete_matrix.mjs`) (2026-09-08)**:
...
- **QA & Verification: Comprehensive Operator QA Test Suite (OP-01 to OP-40), Boundary Matrix Verification & Multi-Tier Bug Discovery (`supabase/tests/test_operator_complete_matrix.mjs`, `AI/OPERATOR_QA_BUG_REPORT.json`, `walkthrough.md`) (2026-09-08)**:
...
- **Bug Fix: /operations — Shift End Timing Validation False Positive on Overnight Shifts, IST Timezone Invariance & Database Overhaul (`packages/utils/src/date.ts`, `packages/validation/src/hourMeter.ts`, `apps/web/app/actions/operators.ts`, `apps/web/components/dashboard/OperatorDashboard.tsx`, `apps/mobile/components/work/MeterLogModal.tsx`, `apps/mobile/app/(app)/operations.tsx`, `supabase/migrations/047_operator_machine_assignments_and_shift_overtime.sql`, `supabase/migrations/050_fix_shift_end_future_validation_and_overnight_derivation.sql`, `supabase/tests/test_future_shift_validation.mjs`) (2026-09-08)**:
  - **1. Bug Confirmation & Multi-Layer Remediation**: Confirmed report as a real bug across client, server, and database layers. On cloud server runtimes (e.g. Vercel) where `process.env.TZ = 'UTC'`, `parseDateTimeToDate` evaluated a 06:00 AM IST shift end as 06:00 AM UTC (11:30 AM IST), triggering false-positive `"Cannot log before shift end."` at 06:10 AM IST (00:40 AM UTC). In PostgreSQL, `submit_operator_hour_log_atomic` lacked timezone-pinned auto-derivation for `v_end_date` (+1 day) and crashed on `SELECT customer_address FROM public.machines` (code 42703). In `apps/web/app/actions/operators.ts`, `rpcError.message?.includes("Cannot log before shift end")` was missing from user validation error handling, falling through to fallback inserts.
  - **2. Shared Packages Timezone Invariance (`packages/utils`, `packages/validation`)**: Formatted parsed dates and times directly with an explicit `+05:30` offset string (`${year}-${mm}-${dd}T${hh}:${mi}:00.000+05:30`) in `parseDateTimeToDate`, rendering epoch milliseconds completely invariant across timezones. Exported `getISTDateString(dateInput?)` for IST-accurate date strings (`YYYY-MM-DD`). Updated `CreateHourLogSchema` refine validator to parse `end_datetime` with timezone awareness.
  - **3. Database Migration & Atomic RPC Overhaul (`supabase/migrations/047_...`, `050_...`)**: Updated `047_operator_machine_assignments_and_shift_overtime.sql` and created standalone migration `050_fix_shift_end_future_validation_and_overnight_derivation.sql`. Auto-derives `v_end_date := v_log_date + INTERVAL '1 day'` when `end_time <= start_time`, derives `v_start_datetime` and `v_end_datetime` in `Asia/Kolkata` (`((...)::timestamp AT TIME ZONE 'Asia/Kolkata')`), enforces `v_end_datetime > NOW() + INTERVAL '1 minute'`, fixes client address resolution from `public.clients`, and inserts fully-resolved timestamps.
  ...
  - **6. Quality Gates & Verification**: Extended `supabase/tests/test_future_shift_validation.mjs` with 13 automated unit, integration, and database RPC tests (13/13 passed, 100%). Monorepo typecheck passed cleanly (`pnpm -r exec tsc --noEmit` exit code 0).
- **Page Feedback: /operations?tab=assignments Operator Card 2-Column Details, Full Shift Timings & Change Operator Action Consolidation (`OperationsClient.tsx`, `operators.ts`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-07)**:
  ...
  - **2. 2-Column Compact Information Layout**: Restructured active operator cards from a single vertical stacked column into a clean 2-column grid (Column 1: Operator Name & Call link; Column 2: Assigned By & Since Date), reducing card height to align with empty slots.
...
- **Page Feedback: /operations?tab=assignments Dedicated Assignment Audit Logs Page & Inline Shift (`/operations/audit-logs`, `AssignmentAuditLogsClient.tsx`, `OperationsClient.tsx`, `assignments.ts`, `AppSidebar.tsx`, `CommandPalette.tsx`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-07)**:
  - **1. Inline Accordion Removal & Footer Link (`apps/web/components/operations/OperationsClient.tsx`)**: Removed `showAllAssignmentsAudit` state, `"View Historical & Audit Logs"` toggle button, and embedded table from `/operations?tab=assignments`. Replaced with clean footer navigation link: `"View Full Assignment Audit Logs →"` pointing to `/operations/audit-logs`. Added `"Assignment Audit Logs"` top tab button in the supervisor navigation bar for seamless direct access.
  - **2. Dedicated Server Route & Aliases (`apps/web/app/(app)/operations/`)**: Created `audit-logs/page.tsx` (Server Component with `requirePermission("machine.view")`, operator protection, and metadata), `assignment-logs/page.tsx` (redirect alias to `/operations/audit-logs`), and updated `page.tsx` route guard to redirect `tab=audit-logs` and `tab=assignment-logs` to `/operations/audit-logs`.
  - **3. High-Density & Responsive Client Component (`AssignmentAuditLogsClient.tsx`)**: Built using Vercel Geist design tokens (`#171717` ink, `#fafafa` canvas, `#ffffff` elevated, `#ebebeb` 1px hairline border). Features 4 KPI summary cards (Total Shifts, Active, Ended, Overnight), multi-criteria filter toolbar (search text, status toggle, shift type, reset), Desktop high-density table (`hidden sm:block`), Mobile touch cards (`block sm:hidden` with ≥44px touch targets), CSV export, and print functions.
  - **4. Resilient Data Access Layer (`assignments.ts`, `queries/index.ts`)**: Created `getAssignmentAuditLogs()` with scalar query projection (`ASSIGNMENT_SCALAR_PROJECTION`), in-memory hydration from parallel-fetched `machines`, `clients`, and `users`, structured error formatting (`formatPostgrestError`), and fallback derivation (`deriveAssignmentsFromMachines`). Exported from `lib/queries/index.ts`.
  - **5. Navigation Integration (`AppSidebar.tsx`, `CommandPalette.tsx`)**: Added `{ label: "Assignment Audit Logs", tab: "audit-logs", href: "/operations/audit-logs" }` to sidebar. Added `nav-operations-assignment-audit` command with keywords `["assignment logs", "shift audit", "operator history", "fleet audit"]` to CommandPalette.
  - **6. Cross-Platform Mobile Parity (`apps/mobile/app/(app)/operations.tsx`)**: Added `'audit-logs'` to `OpsTab`, added `"Audit Logs"` segment button in supervisor tab bar, implemented `MobileAssignmentAuditRecord`, full query with fallback, and mobile touch card feed.
  - **7. Quality Gate Verification**: `pnpm -r exec tsc --noEmit` passed with 0 errors across all 7 workspace packages.
- **Page Feedback: /operations?tab=logs — Subnavigation & Page Tab Icon Removal Across Monorepo (`AppSidebar.tsx`, `NavigationItem.tsx`, `CollapsedSidebarFlyout.tsx`, `MobileSidebarDrawer.tsx`, `types.ts`, `OperatorDashboard.tsx`, `OperationsClient.tsx`, `ReportsClient.tsx`, `HRClient.tsx`, `CrmClient.tsx`, `ClientDetailClient.tsx`, `FinanceClient.tsx`, `VendorDetailClient.tsx`, `PODetailClient.tsx`, `MobileDueBuckets.tsx`, `navItems.ts`, `CustomBottomTabBar.tsx`, `DynamicBottomNav.tsx`) (2026-09-07)**:
  - **1. Sidebar Subnavigation Icon Pruning (`apps/web`)**:
    - Made `icon?: React.ComponentType<{ className?: string }>` optional on `SubNavItem` in `apps/web/components/layout/sidebar/types.ts`.
    - Removed `icon` property from all `subItems` across modules in `AppSidebar.tsx` (`Machines`, `Operations`, `Clients`, `Employees & Users`, and operator dynamic `Log Entry`/`Log History` sub-items), keeping top-level parent menu icons intact.
    - Removed `<SubIcon ... />` rendering inside `SidebarMenuSubButton` in `NavigationItem.tsx`.
    - Removed `<SubIcon ... />` rendering from flyout submenu links in `CollapsedSidebarFlyout.tsx`.
    - Removed `<SubIcon ... />` rendering from mobile drawer sub-items in `MobileSidebarDrawer.tsx`.
  - **2. Operations Hub & Dashboard Tabs (`apps/web`)**:
    - Streamlined top tabs in `OperationsClient.tsx` to text with badge indicators only (`Daily Running Hours`, `Operator Machine Assignments`); pruned unused `AnimatedStar` import.
  ...
  - **3. Page-Level Tabs Across Monorepo Modules (`apps/web`)**:
    ...
    - `HRClient.tsx`: Pruned `icon` and `<Icon size={14} />` from all 8 navigation sub-tabs (`HR Dashboard`, `Employees`, `Onboarding`, `Departments`, `Designations`, `Salary & Payroll`, `User Requests`, `Documents`).
...
- **Bug Fix: /operations/audit-logs — "[DAL] Error fetching assignment audit logs: {}" Console Error Remediation & Resilient In-Memory Hydration Query Architecture (`apps/web/lib/queries/assignments.ts`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-07)**:
  - **1. PostgREST Multiple-FK Disambiguation & Error Serialization Fix**: Identified that `getAssignmentAuditLogs` in `apps/web/lib/queries/assignments.ts` failed due to PostgREST relationship cache resolution failures on 3 simultaneous foreign keys to `public.users` (`operator_id`, `assigned_by`, `ended_by`) plus nested `client:clients` within `machine:machines`. Replaced raw `console.error` with `formatPostgrestError`, eliminating Next.js Turbopack dev server IPC bugs that serialized unformatted Error objects into empty `{}` across server-to-browser logging.
  - **2. Safe Scalar Projection & In-Memory Hydration (`apps/web/lib/queries/assignments.ts`)**: Replaced fragile multi-FK joins with `ASSIGNMENT_SCALAR_PROJECTION` (`id, machine_id, operator_id, shift_start_time, shift_end_time, crosses_midnight, is_active, assigned_by, assigned_at, ended_at, ended_by, end_reason, created_at, updated_at`). Parallel-loaded `machines`, `clients`, `users`, and `operator_machine_assignments`. Hydrated machine (with client details), operator, assigner, and ender in memory via lookup maps with zero foreign-key schema resolution risk.
  - **3. Resilient Fallback Machine Derivation (`deriveAssignmentsFromMachines`)**: If `operator_machine_assignments` is empty or unmigrated, automatically derives assignment audit records from current machine state (`current_operator_id`, `operator_ids`, `current_supervisor_id`), ensuring the audit log page and metrics never display empty/zeroed state.
  - **4. Mobile React Native Parity (`apps/mobile/app/(app)/operations.tsx`)**: Synchronized resilient assignment query fallback: if primary assignments query fails on mobile, automatically executes scalar query and hydrates in memory from machines and operators, and derives active assignments from machines if table is unpopulated.
- **Page Feedback: /operations?tab=assignments UI/UX Polish, Card Normalization & Expandable Accordion Architecture (`OperationsClient.tsx`, `operators.ts`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-07)**:
  - **1. Subtitle & Tab Button Icon Removal**: Removed the paragraph `"Manage recurring daily shifts, active operator rosters..."` below the page heading. Removed the `<Clock>` SVG graphic from the `"Daily Running Hours"` tab button. Removed the `<AnimatedStar>` icon from the `"Operator Machine Assignments"` tab button.
  - **2. Page Title Normalization & Action Alignment**: Changed the main page title to `"Fleet Operations"` to eliminate redundancy with the tab title. Formatted `"Assign Operator"` button with `inline-flex items-center gap-2 whitespace-nowrap` guaranteeing strictly 1-line alignment between `<AnimatedUserCheck>` and the text label across all viewport sizes.
  - **3. Expandable Machine Cards (Closed by Default)**: Added `expandedMachineIds` state initialized to `new Set()` (closed by default). Built a compact summary strip when collapsed showing machine ID, model, serial, meter hours, status badge, operator capacity indicator (`X / 3 Operators`), quick-reference operator chips (`Day: Rajesh Kumar`), direct `+ Assign` button if capacity is available, and chevron toggle. Added "Expand All / Collapse All Cards" utility in the filter toolbar.
  - **4. Expanded Slot Information & Dedicated Actions**: In expanded view, displays shift badge with morning/day/night icons and hours, operator name, clickable phone call link (`tel:[phone]`), supervisor assigner attribution (`Assigned by: [Supervisor Name] • [Date]`), and 3 clean action buttons: "Change Operator" (reassign), "End Shift" (`shift_changed`), and "Unassign" (`removed`).
  - **5. Relational Assigner Enrichment (`apps/web/lib/queries/operators.ts`)**: Updated `getOperationsHubData` to parallel-fetch all staff/supervisor profiles and map `assigner` details directly onto active assignments in memory, preventing PostgREST schema cache join failures.
  - **6. Mobile React Native Parity (`apps/mobile/app/(app)/operations.tsx`)**: Synchronized accordion cards with default-closed condition, quick chips, supervisor attribution, telephone calling via `Linking.openURL`, and 3 touch buttons (Change, End Shift, Unassign).
- **Bug Fix: /operations — "Error fetching supervisor hour logs: {}" Console Error Remediation & Resilient Multi-Tier Hour Logs Query Architecture (`apps/web/lib/queries/operators.ts`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-07)**:
  - **1. PostgREST Foreign Key Disambiguation & Error Formatting**: Identified and removed ambiguous `supervisor:users!machine_hour_logs_supervisor_id_fkey` join from `HOUR_LOG_FULL_PROJECTION` in `apps/web/lib/queries/operators.ts` (supervisors are already fetched and hydrated in memory). Replaced raw `console.error(..., error)` with `formatPostgrestError`, eliminating Turbopack dev server error serialization bugs that forwarded Error instances as empty `{}` objects across the server-client IPC boundary.
  - **2. Resilient Multi-Tier Fallback Query Pipeline (`apps/web/lib/queries/operators.ts`)**: Implemented `fetchHourLogsResiliently` with a 3-tier cascade: Tier 1 (`HOUR_LOG_FULL_PROJECTION` with conflict extensions), Tier 2 (`HOUR_LOG_BASE_PROJECTION` without migration 047 conflict columns matching `reports.ts`), and Tier 3 (`HOUR_LOG_DIRECT_PROJECTION` pure scalar columns). Updated `formatHourLogsData` to accept parallel-loaded `machines`, `staffUsers`, and `clients` collections and automatically enrich relational objects in memory if PostgREST schema cache joins fail. Applied to both Supervisor Hub and Operator Daily Entry Tab.
  - **3. Mobile React Native Parity (`apps/mobile/app/(app)/operations.tsx`)**: Added automatic fallback to baseline projection without conflict fields if primary hour logs query fails on mobile. Enriched warning logging with `logsRes.error?.message || logsRes.error`.
- **Page Feedback: /onboarding Schedule Text Removal & /signup Address Field Integration (Web, Mobile & Database) (`OnboardingClient.tsx`, `signup/page.tsx`, `actions/auth.ts`, `validation/auth.ts`, `apps/mobile/app/(auth)/signup.tsx`, `049_add_address_to_signup_and_user_profile.sql`) (2026-09-07)**:
  - **1. Onboarding Screen UI Polish (`apps/web/app/onboarding/OnboardingClient.tsx`)**: Removed explanatory paragraph `<p className="text-[11px] text-[var(--color-mute)] leading-normal">Standard daily operational schedule. Used to configure equipment shift assignments and calculate overtime.</p>` below the shift time pickers in Section 2 (Work Shift Schedule), leaving clean, streamlined `<CustomTimePicker>` controls.
  - **2. Web Registration Street Address Integration (`apps/web/app/signup/page.tsx`)**: Added `address: ""` to `formValues` state. Integrated canonical `<Input>` for "Street / Site Base Address" (`id="signup-address"`) in Section 3 (Work Location & Identity) right below the City/District/State grid with placeholder `"Plot No. 42, MIDC Industrial Area, Chakan"`, required red star, and `<AnimatedMapPin size={15} />`. Added pre-flight client-side validation and preserved `address` in `result.fieldValues` state hydration on response.
  - **3. Auth Server Action Mutation & Metadata (`apps/web/app/actions/auth.ts`)**: Extracted `address` from incoming `FormData`, enforced required non-empty check returning `fieldErrors.address`, retained input in `fieldValues`, and passed `address` in `options.data` and formatted `location` during `supabase.auth.signUp()`.
  - **4. Shared Canonical Validation Schemas (`packages/validation/src/auth.ts`)**: Added required `address: z.string().trim().min(3, "Street / site base address is required").max(255, "Address cannot exceed 255 characters")` to `SignupSchema`, and optional `address` to `CreateUserSchema` and `UpdateUserSchema`.
  - **5. Mobile React Native App Parity (`apps/mobile/app/(auth)/signup.tsx`)**: Added `address` state, client-side validation, `Street / Site Base Address *` `<Input>` with MapPin icon and placeholder in Section 3, and passed `address: address.trim()` in `supabase.auth.signUp()` options data.
  - **6. Database Schema & Trigger (`supabase/migrations/049_add_address_to_signup_and_user_profile.sql`, `048_add_complete_profile_and_onboarding_workflow.sql`)**: Ensured `public.users.address TEXT` column. Updated `handle_new_user()` trigger on `auth.users` to capture `v_address := NULLIF(NEW.raw_user_meta_data->>'address', '')`, store it into `public.users.address`, and require `v_address` for setting `v_complete_profile = 'yes'`. Fixed PostgreSQL `invalid input syntax for type json` in `complete_user_onboarding_atomic`: replaced unquoted string in `audit_logs.details` with valid `jsonb` object `v_audit_meta`.
  - **7. Resilient Direct Table Persistence Fallback (`apps/web/app/actions/onboarding.ts`)**: Added `createSupabaseAdminClient()` fallback in `completeOnboardingAction`. If the RPC fails or hasn't been reloaded in the remote database, the server action directly updates all user profile fields (`full_name`, `phone`, `role`, `shift_time`, `address`, `city`, `district`, `state`, `state_id`, `aadhaar_number`, `license_number`, `complete_profile: 'yes'`), guaranteeing 100% data persistence without blocking the user.
  - **8. Monorepo Quality Gate Verification**: `pnpm -r exec tsc --noEmit` passed with 0 errors across all 7 workspace packages; `pnpm --filter @reachinternational/web build` passed with all 39/39 routes compiled cleanly (Exit code 0).
- **Monorepo Architecture Simplification & Dead Code Pruning — Ponytail Review (`OperationsClient.tsx`, `assignments.ts`, `onboarding.ts`, `auth.ts`, `date.ts`, `dal.ts`) (2026-09-07)**:
  - **1. Operations Hub Pruning (`OperationsClient.tsx`, `page.tsx`)**: Removed unused action imports `hireOperatorAction`, `recordOperatorPayoutAction`, and `recordMachineSiteMovementAction`. Shrunk `formatMachineSelectLabel` from 28 lines of nested conditionals down to a 7-line template string. Deleted 3 orphan modals (Hire Operator, Site Movement, Record Payout) and 18 unused state variables for soft-removed tabs (~280 lines of dead JSX). Deleted dead TAB 3 and TAB 4 rendering blocks, removing `siteMovements` and `operatorPayouts` references.
  - **2. Server Actions RPC Fallback Elimination (`assignments.ts`, `onboarding.ts`)**: Removed unused imports `getCurrentUser` and `logAudit` from `assignments.ts`. Eliminated ~180 lines of manual table operations and fallback checks duplicating atomic database RPCs (`assign_operator_machine_atomic`, `end_operator_machine_assignment_atomic`, and `resolve_hour_log_conflict_atomic`). Shrunk 40 lines of manual `formData.get` in `onboarding.ts` with `Object.fromEntries(formData.entries())`. Deduplicated manual Aadhaar and licence checks in `onboarding.ts` handled by `OnboardingProfileSchema`. Deleted redundant direct table update fallback in `onboarding.ts`.
  - **3. Validation & Utilities Deduplication (`auth.ts`, `date.ts`, `string.ts`)**: Replaced copy-pasted Verhoeff algorithm matrices (`VERHOEFF_D`, `VERHOEFF_P`, `validateVerhoeff`) and `INDIAN_STATE_CODES` (~140 lines) by importing `validateAadhaarNumber`, `validateLicenseNumber`, and `INDIAN_STATE_CODES` from `@reachinternational/utils`. Shrunk `AadhaarFieldSchema` and `AadhaarRequiredFieldSchema` to single-line delegations. Deleted unused `isTimeWindowOverlapping` (34 lines) from `packages/utils/src/date.ts`. Exported `INDIAN_STATE_CODES` from `packages/utils/src/string.ts`.
  - **4. Auth DAL Simplification (`apps/web/lib/dal.ts`)**: Shrunk `isProfileIncomplete` from 23 lines down to a single-line check: `user.complete_profile !== "yes"`.
  - **5. Quality Gates & Metrics**: `pnpm -r exec tsc --noEmit` passed with 0 errors across all monorepo packages; `pnpm --filter @reachinternational/web build` passed with all 39/39 routes compiled cleanly (Exit code 0). Net impact: **-1,026 lines deleted**, **+147 lines added** (`net: -879 lines`).
- **User Profile Onboarding Architecture for Incomplete Profiles (Web, Mobile & Database) (`048_add_complete_profile_and_onboarding_workflow.sql`, `onboarding/page.tsx`, `OnboardingClient.tsx`, `apps/mobile/app/(auth)/onboarding.tsx`, `actions/onboarding.ts`, `dal.ts`, `useAuth.tsx`, `validation/auth.ts`, `database.ts`) (2026-09-07)**:
  - **1. Database Schema & Migration (`supabase/migrations/048_add_complete_profile_and_onboarding_workflow.sql`)**: Added `complete_profile TEXT NOT NULL DEFAULT 'no' CHECK (complete_profile IN ('yes', 'no'))` to `public.users` with partial index `idx_users_incomplete_profile ON public.users(complete_profile) WHERE complete_profile = 'no'`. Automated backfill of existing complete user profiles to `complete_profile = 'yes'`. Updated `handle_new_user()` trigger to automatically evaluate completeness on new signups. Implemented atomic RPC `complete_user_onboarding_atomic` with transaction isolation, input validation, and structured audit log recording (`user.onboarding_completed`).
  - **2. Fast Zero-Latency Auth Bypass (`apps/web/lib/dal.ts`, `apps/web/proxy.ts`, `apps/web/app/(app)/layout.tsx`)**: Cached `complete_profile` column in DAL `getCachedUserRow` (`dal-user-row-v7`). Added `isProfileIncomplete(user: User): boolean` helper checking mandatory profile fields (`full_name`, `phone`, `role`, `shift_time`, `address`, `city`, `district`, `state`, `aadhaar_number`). Guarded `(app)/layout.tsx`: when `user.complete_profile === 'yes'`, checks resolve in 1 single string check with **0 CPU overhead**, immediately bypassing validation; only when `user.complete_profile !== 'yes'` does it evaluate missing fields and redirect to `/onboarding`. Protected `/onboarding` in `proxy.ts` while allowing authenticated access.
  - **3. Shared Types & Schema Validation (`packages/*`)**: Extended `User` in `packages/types/src/database.ts` with `complete_profile?: 'yes' | 'no' | string | null;`. Created `OnboardingProfileSchema` and `OnboardingProfileInput` in `packages/validation/src/auth.ts` with Verhoeff Aadhaar checksum validation and driving licence checks.
  - **4. Web Onboarding Screen & Server Action (`apps/web/app/onboarding/`)**: Created Server Component `apps/web/app/onboarding/page.tsx` verifying session and redirecting completed users. Created Client Component `apps/web/app/onboarding/OnboardingClient.tsx` reusing `/signup` components (industrial equipment showcase panel, 4 semantic sections with numbered badge indicators, `<CustomTimePicker>` with live shift duration badge, `<SearchableSelect>`, Aadhaar auto-formatting, and dynamic progress bar). Created Server Action `apps/web/app/actions/onboarding.ts` (`completeOnboardingAction`) with atomic RPC call, direct table fallback, cache tag revalidation (`CACHE_TAGS.users, "max"`), and role-based redirect.
  - **5. Mobile React Native Parity (`apps/mobile/lib/auth/useAuth.tsx`, `apps/mobile/app/(auth)/onboarding.tsx`, `(app)/_layout.tsx`)**: Updated `useAuth.tsx` to query `complete_profile` and expose `isProfileComplete` and `userProfile`. Guarded `apps/mobile/app/(app)/_layout.tsx` to redirect to `/(auth)/onboarding` if profile is incomplete. Created native `apps/mobile/app/(auth)/onboarding.tsx` with identical 4 sections, `<TimeInput>` pickers, states modal, 44px touch targets, and atomic submission.
  - **6. Monorepo Quality Gate Verification**: `pnpm -r exec tsc --noEmit` passed with 0 errors across all 7 workspace packages; `pnpm --filter @reachinternational/web build` passed with all 39/39 routes compiled cleanly including `ƒ /onboarding` (Exit code 0); `README.md` updated with new schema and module documentation.
- **Add Strix Domain Verification File (`apps/web/public/.well-known/strix-verify.txt`) (2026-09-07)**:
  - Added static domain verification file `apps/web/public/.well-known/strix-verify.txt` with content `strix-verify-1c75e87c52d3e647f2f862a63116f275` served at `/.well-known/strix-verify.txt` on Vercel deployment (`dashboard-reachinternational.vercel.app`).
- **Monorepo Architecture Pruning & Over-Engineering Elimination — Ponytail Audit (`packages/*`, `apps/mobile/*`, `apps/web/*`) (2026-09-07)**:
  - **1. Package Pruning**: Deleted empty placeholder package `@reachinternational/config` and unused `@reachinternational/api-client` package (0 callers across web and mobile). Reduced active monorepo packages from 9 to 7 (`@reachinternational/web`, `@reachinternational/mobile`, `@reachinternational/types`, `@reachinternational/utils`, `@reachinternational/design-tokens`, `@reachinternational/permissions`, `@reachinternational/validation`).
  - **2. Shared Packages Cleanup (`packages/*`)**: Deleted 48 lines of hand-rolled `groupBy`, `sortBy`, `filterNil`, `omit`, `pick` from `packages/utils/src/object.ts` (replaced by native `Object.groupBy` and `Array.prototype.toSorted`). Standardized `formatCompactCurrency` in `packages/utils/src/currency.ts` to native `Intl.NumberFormat('en-IN', { notation: 'compact' })` and deleted unused `calculateGST`. Removed dead regex template substitution engine `renderSafeTemplate` from `packages/utils/src/ssti.ts` while preserving active `escapeHtml`. Deleted unused query schemas in `packages/validation/src/common.ts` (31 lines) and unused `validateData` / `formatZodError` wrappers in `packages/validation/src/helpers.ts` (22 lines).
  - **3. Mobile App Bloat & Mock Engine Removal (`apps/mobile`)**: Deleted in-app test runner `runUnitTestSuite` from client runtime (`apps/mobile/lib/testing-suite.ts` - 148 lines). Deleted mock offline queue engine with simulated `setTimeout` sync (`apps/mobile/lib/offline-sync.ts` - 136 lines) and unmounted `OfflineSyncBanner.tsx` (50 lines). Deleted unused Supabase Realtime channel subscription manager (`apps/mobile/lib/realtime.ts` - 116 lines), multi-environment manager (`apps/mobile/lib/environment.ts` - 92 lines), accessibility helper (`apps/mobile/lib/accessibility.ts` - 48 lines), push token registration stub (`apps/mobile/lib/notifications.ts` - 42 lines), and redundant `OptimizedList.tsx` wrapper + `performance.ts` (87 lines).
  - **4. Web App Directory Flattening & Duplicate Clients (`apps/web`)**: Flattened 4-level deep `components/animate-ui/components/radix/dialog.tsx` into canonical `components/ui/dialog.tsx` and deleted `components/animate-ui/`. Deleted orphaned duplicate Supabase clients (`lib/server.ts`, `lib/client.ts`, `lib/supabase/browser.ts`) and unused structured telemetry logger (`lib/telemetry.ts`). Deleted pass-through alias wrappers `DatePicker.tsx` and `TimePicker.tsx`. Inlined `roleHasPermission` in `apps/web/app/(app)/hr/page.tsx` and removed single-caller helper `lib/auth/server-rbac.ts`. Inlined direct `@reachinternational/permissions` imports across 6 callers and deleted pass-through barrel files `lib/auth/scope.ts` and `lib/auth/rbac.ts`. Removed unused `cacheWithTag` from `lib/cache.ts` while preserving active `CACHE_TAGS`.
  - **5. Dependency Pruning**: Removed `shadcn` CLI from runtime dependencies in `apps/web/package.json`. Removed unused `@base-ui/react` from `apps/web/package.json` and `next.config.ts`. Removed workspace dependency `@reachinternational/api-client` from `apps/web` and `apps/mobile`. Moved `@supabase/supabase-js` to devDependencies in root `package.json`. Pruned 157 transitive/direct packages from `node_modules` and saved ~1,450 lines of code.
  - **6. Quality Gate Verification**: `pnpm typecheck` passed with 0 errors across all 7 workspace packages; `pnpm --filter @reachinternational/web build` passed with all 38/38 routes compiled cleanly (Exit code 0); `pnpm --filter @reachinternational/mobile build` passed cleanly; `README.md` updated.
- **Operator–Machine Assignment, Shift Timings & Overtime Conflict Resolution Architecture (`047_operator_machine_assignments_and_shift_overtime.sql`, `047_backfill_operator_assignments.sql`, `OperationsClient.tsx`, `operations.tsx`, `MobileAssignmentModal.tsx`, `MobileConflictResolutionModal.tsx`, `assignments.ts`, `database.ts`, `machine.ts`, `date.ts`, `endpoints/index.ts`, `test_operator_machine_assignments.mjs`) (2026-09-07)**:
  - **1. Database Migration & GiST Exclusion (`supabase/migrations/047_operator_machine_assignments_and_shift_overtime.sql`)**: Implemented recurring daily shift model (`shift_start_time`, `shift_end_time` `TIME`) in `public.operator_machine_assignments` with transaction-level advisory locks enforcing max 3 distinct active operators per machine (`enforce_max_operators_per_machine` using `COUNT(DISTINCT operator_id)` without penalizing split shifts). Normalizes shifts to 0–1440 minute circular ranges unrolling across midnight with PostgreSQL GiST exclusion constraint (`EXCLUDE USING GIST (operator_id WITH =, minute_range WITH &&) WHERE (is_active)`) in `public.operator_shift_ranges`. Maintained `machines.operator_ids` and `machines.current_operator_id` as trigger-synced read mirrors for 100% backward compatibility.
  - **2. Non-Blocking Overtime Conflict Soft-Flagging & Resolution**: Extended `public.machine_hour_logs` with `conflict_flag`, `conflict_reason`, `conflict_status`, `conflict_resolved_by`, `conflict_resolved_at`, and `conflict_resolution_notes`. Overtime exceeding shift into another active assignment is soft-flagged using circular range overlap (`shift_to_ranges()` + `&&`) without blocking field log submissions. Fixed `audit_logs` column mismatch in `submit_operator_hour_log_atomic` to use canonical `(user_id, action, entity_type, entity_id, metadata, details, created_at)`. Created `resolve_hour_log_conflict_atomic` supporting "Acknowledge" (keep log as-is) and "Adjust" (trim end time) resolution actions.
  - **3. Standalone Backfill Script with Dry-Run Report Mode (`supabase/scripts/047_backfill_operator_assignments.sql`)**: Decoupled Step 12 historical backfill into a standalone administrative script equipped with default DRY-RUN report mode (`v_dry_run := true`) using structured `RAISE NOTICE` logging. Parses operator profile shifts (`public.users.shift_time`) with fallback to staggered 8-hour windows (`08:00-16:00`, `16:00-00:00`, `00:00-08:00`). Detects and reports double-booking conflicts and machine capacity limits (>3/machine) before live insertion.
  - **4. Shared Packages & API Contracts (`packages/*`)**: Defined `OperatorMachineAssignment` and extended `MachineHourLog` in `@reachinternational/types`. Created `CreateAssignmentSchema`, `EndAssignmentSchema`, and `ResolveConflictSchema` in `@reachinternational/validation`. Added circular shift range math and normalization helpers in `@reachinternational/utils`. Extended `@reachinternational/api-client` with assignment & conflict resolution contracts.
  - **5. Web Server Actions & DAL (`apps/web/app/actions/assignments.ts`, `lib/queries/*`)**: Implemented `createAssignmentAction`, `endAssignmentAction`, `resolveHourLogConflictAction`, `getOperatorProfileShiftAction`, `getMachineActiveAssignmentsAction` with resilient direct table fallbacks (`PGRST202` handling) ensuring 100% continuous uptime. Hydrated `active_assignments` in machine and operations hub queries with granular cache tag revalidation.
  - **6. Web Operations Hub (`apps/web/components/operations/OperationsClient.tsx`)**: Upgraded Operator Machine Assignments tab with Multi-Shift Equipment Roster Cards (capacity indicator `X/3`, daytime `☀️` and overnight `🌙` shift badges, operator contact, Reassign, End Shift, and + Assign Shift slot buttons). Added Overtime Conflict Alert Banner with 1-click Review modal. Upgraded assignment modal with profile shift auto-fill, mandatory supervisor shift selection when profile shift is null, and `<CustomTimePicker>` integration.
  - **7. Mobile React Native Parity (`apps/mobile/app/(app)/operations.tsx`, `MobileAssignmentModal.tsx`, `MobileConflictResolutionModal.tsx`)**: Created native bottom sheet assignment modal with capacity display, profile shift pre-fill, Start/End time `<TimeInput>` pickers, and native overtime conflict resolution modal. Synchronized multi-shift equipment cards with capacity pills (`X/3`), day/overnight badges, and 44px min touch buttons.
  - **8. Monorepo Quality Gate Verification**: `node supabase/tests/test_operator_machine_assignments.mjs` passed (100%); `pnpm typecheck` (`turbo run typecheck`) passed with 0 errors across all 9 packages; `pnpm --filter @reachinternational/web build` passed with all 38/38 routes compiled cleanly (Exit code 0); `README.md` updated with new schemas.
- **Page Feedback: /signup — Header & Section Clutter Removal, Clock Icon Removal, and Mandatory Shift Start & End Time with Red Star (`signup/page.tsx`, `CustomTimePicker.tsx`, `auth.ts`, `actions/auth.ts`, `apps/mobile/app/(auth)/signup.tsx`) (2026-09-07)**:
  - **1. Header & Section Clutter Removal (`apps/web/app/signup/page.tsx`)**: Removed Employee Registration badge (`.inline-flex`) from the card header. Removed subtitle paragraph (`Fill in your details and shift timings to request workspace platform access.`) below `Create an account`, leaving a clean, high-impact header. Removed `"Personal details"` subtitle from Section 1 (Account & Role) header. Removed `"Verification documents"` subtitle from Section 3 (Work Location & Identity) header. Removed `"Access protection"` subtitle from Section 4 (Security Credentials) header. Removed unused `ShieldCheck` icon import.
  - **2. Clock Icon Removal from CustomTimePicker (`apps/web/components/ui/CustomTimePicker.tsx`, `apps/web/app/signup/page.tsx`)**: Extended `CustomTimePickerProps` with `hideIcon?: boolean` and `showIcon?: boolean`. Conditionally rendered Lucide `<Clock>` icon in label (`{!hideIcon && showIcon && <Clock ... />}`). Added `hideIcon` to both Shift Start Time and Shift End Time pickers in `page.tsx`.
  - **3. Mandatory Shift Start & End Time with Red Star (`apps/web/app/signup/page.tsx`, `CustomTimePicker.tsx`, `packages/validation/src/auth.ts`, `apps/web/app/actions/auth.ts`)**: Passed `required` to Shift Start Time and Shift End Time `<CustomTimePicker>`s, displaying the red star `<span className="text-rose-500 font-semibold ml-0.5 shrink-0">*</span>`. Added client-side validation in `handleSubmit` checking that `shift_start_time` and `shift_end_time` are not empty before submission. Enforced backend validation in Server Action `signup()` in `actions/auth.ts`, setting `fieldErrors.shift_start_time` and `fieldErrors.shift_end_time`. Updated `SignupSchema` in `packages/validation/src/auth.ts` requiring `shift_start_time` and `shift_end_time` (`min(1, "Shift ... time is required")`).
  - **4. Cross-Platform Mobile App Synchronization (`apps/mobile/app/(auth)/signup.tsx`)**: Removed `Step X of 4` step indicator text from Section 1, Section 3, and Section 4 headers for clean section layout. Added `required` to Shift Start Time and Shift End Time `<TimeInput>` components (rendering red star `*`). Added validation in `handleSignup()` ensuring `shiftStartTime` and `shiftEndTime` are provided.
  - **5. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 workspace packages with 0 errors; `pnpm --filter @reachinternational/web build` passed with all 38/38 routes compiled cleanly including `○ /signup` (Exit code 0).
- **Page Feedback: /signup — Clean, Structured & User-Friendly UI/UX with Shift Start & End Time Pickers (Web & Mobile) (`signup/page.tsx`, `apps/mobile/app/(auth)/signup.tsx`, `CustomTimePicker.tsx`, `actions/auth.ts`, `auth.ts`) (2026-09-07)**:
  - **1. Web Registration Semantic 4-Section Architecture (`apps/web/app/signup/page.tsx`)**: Re-architected the form from an unbroken wall of 12 flat inputs into 4 purposeful, beautifully structured sections with hairline borders, elevated background tokens, and numbered pill badges: Section 1 (Account & Role: Full Name, Email, Phone, Role), Section 2 (Work Shift Schedule: Shift Start & End Time pickers via canonical `<CustomTimePicker>`, dynamic duration calculation badge via `computeShiftTiming()`, and helper text), Section 3 (Work Location & Identity: 3-column City/District/State and 2-column Aadhaar/Licence), Section 4 (Security Credentials: Password and Confirm Password). Polished header with employee registration badge and subtitle. Fixed Note banner typo (`"Note: "` with proper space) and added Lucide `<Info>` icon. Preserved hidden inputs and calibrated padding for 1366×599 laptop screens.
  - **2. Cross-Platform Mobile App Synchronization (`apps/mobile/app/(auth)/signup.tsx`)**: Synchronized matching 4-section architecture (`styles.sectionContainer`, numbered badges `1` to `4`, hairline dividers, live shift duration badge `☀️ / 🌙 {durationFormatted}` via `computeShiftTiming()`, and Lucide `Info` notice box).
  - **3. Backend Server Actions & Data Access Layer (`apps/web/app/actions/auth.ts`)**: Extracted `shift_start_time`, `shift_end_time`, and `shift_time` from incoming `FormData`. Formatted canonical shift timing string (`"08:00 AM - 08:00 PM"`). Passed `shift_time`, `shift_start_time`, and `shift_end_time` into `options.data` during `supabase.auth.signUp()`, which database trigger `handle_new_user()` (migration 042) automatically inserts into `public.users.shift_time`. Preserved values in `fieldValues` on error and recorded `shift_time` in structured audit log metadata (`user.signup`).
  - **4. Shared Schema Validation (`packages/validation/src/auth.ts`)**: Extended `SignupSchema` with optional `shift_time`, `shift_start_time`, and `shift_end_time` validation rules.
  - **5. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors; `pnpm --filter @reachinternational/web build` passed with all 38/38 routes compiled cleanly including `○ /signup` (Exit code 0).
- **Page Feedback: /operations?tab=logs — Client Selection Fix, Default 'All Machines' & Client-Wise Fleet and Log Filtering (`ClientSelect.tsx`, `OperationsClient.tsx`, `PrintableSupervisorLogsModal.tsx`, `supervisor-logs-export.ts`) (2026-09-05)**:
  - **1. Resilient Client Selection Architecture (`ClientSelect.tsx`, `OperationsClient.tsx`)**: Resolved the issue where clicking a client left the trigger button showing `"Select Client..."` due to ID vs name mismatches. Updated `ClientSelect.tsx` to match selected clients flexibly by `c.id === value`, `c.client_name === value`, `c.company_name === value`, `c.name === value`, or `c.code === value`. In `OperationsClient.tsx`, transitioned state management to `logsSelectedClientId` backed by `allClientsList` (combining `dbClients` and runtime log entities). Defaulted active client selection to clients with active hour logs (e.g. Saint Gobain with 31 logs) or the first available client so supervisors immediately see data.
  - **2. Default 'All Machines' Selection (`OperationsClient.tsx`)**: Addressed user feedback requesting `"by default select all machine"`. Set `allowAll={true}` and `allLabel="All Machines"` in `<MachineSelect>` under Client view mode. Configured `effectiveSelectedClientMachineId` to default to `"all"`, displaying the full fleet count badge (e.g. `18 Total` or `1 Total`) and eliminating the empty `"Search or select machine..."` placeholder.
  - **3. Complete Client Fleet Discovery & Client-Wise Log Filtering (`OperationsClient.tsx`, `PrintableSupervisorLogsModal.tsx`, `supervisor-logs-export.ts`)**: Re-architected `clientMachines` derivation to check `m.client_id === activeClientId`, `(m as any).client?.id === activeClientId`, and company name comparisons, ensuring all 18 machines rented to JK Paper Ltd. appear in the machine select dropdown. Enhanced `filteredHourLogs` in Client mode to match by `log.client_id`, `log.client.id`, company name, and machine fleet ownership. Synchronized client matching across the web data table, printable supervisor report modal (`PrintableSupervisorLogsModal.tsx`), and Excel spreadsheet export (`supervisor-logs-export.ts`).
  - **4. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit`: Passed with 0 errors; `pnpm -r exec tsc --noEmit`: Passed across all 9 workspace packages with 0 errors; `pnpm --filter @reachinternational/web build`: Passed with all 35/35 routes compiled cleanly (code 0). Automated regression tests passed: `test_future_shift_validation.mjs` (9/9 passed, 100%), `test_breakdown_submission_and_linking.mjs` (all passed, 100%).
- **Page Feedback: /operations?tab=entry — Complete Web Sidebar Architecture Rewrite & Zero-Jitter Refresh Persistence (`ThemeScript.tsx`, `layout.tsx`, `sidebar.tsx`, `AppShellClient.tsx`) (2026-09-05)**:
  - **1. Pre-Boot DOM & Cookie Synchronization (`apps/web/components/theme/ThemeScript.tsx`)**: Synchronously evaluates `reachinternational_sidebar_collapsed` in `localStorage` and fallback `document.cookie` before `<body>` renders. If collapsed (`true`): immediately adds class `sidebar-collapsed` and sets `--sidebar-width: 72px` on `document.documentElement`. If expanded (`false`): removes `sidebar-collapsed` and sets `--sidebar-width: 280px`. Synchronously maintains `document.cookie` (`path=/; max-age=31536000; SameSite=Lax`). Guarantees the browser receives and parses the exact sidebar width on Frame 0 before any React code executes, preventing any visual flash or FOUC.
  - **2. Dynamic SSR Cookie Hydration (`apps/web/app/(app)/layout.tsx`)**: Added `export const dynamic = "force-dynamic";` and `export const revalidate = 0;`. Ensures Next.js App Router always evaluates authenticated app layouts dynamically and never serves a stale cached layout chunk with outdated cookie state. Reads `reachinternational_sidebar_collapsed` cookie safely via `await cookies()` and passes `defaultCollapsed={sidebarCookie === "true"}` to `<AppShellClient>`.
  - **3. Re-architected Core Sidebar UI Primitives (`apps/web/components/ui/sidebar.tsx`)**: Replaced `<motion.aside animate={{ width: ... }}>` with a pure `<aside>` element, completely eliminating Framer Motion hydration width tweening that caused the expand-then-close glitch. Applied GPU-accelerated CSS transitions `transition-[width] duration-200 ease-[cubic-bezier(0.2,0.8,0.2,1)]` guarded by `isInteractive`. Transitions are strictly disabled (`transition-none`) on initial mount and page refresh, rendering stiffly with 0ms animation, while user-initiated clicks glide smoothly at 200ms. Sized explicitly via `style={{ width: collapsed ? SIDEBAR_WIDTH_COLLAPSED : SIDEBAR_WIDTH_EXPANDED }}`. Preserved 100% of the visual styling, Geist tokens, and keyboard shortcut (`Ctrl+B` / `Cmd+B`).
  - **4. Cohesive State Management in App Shell (`apps/web/components/layout/AppShellClient.tsx`)**: Initialized `collapsed` state strictly with `defaultCollapsed` from SSR. Added `isInteractive` guard: `md:pl-[var(--sidebar-width)]` on the main workspace wrapper uses `isInteractive ? "transition-[padding] duration-200 ease-[cubic-bezier(0.2,0.8,0.2,1)]" : "transition-none"`, preventing horizontal content sliding on page load. Implemented atomic `saveSidebarState(next: boolean)` updating `localStorage`, `document.cookie`, `document.documentElement.classList`, and CSS variable `--sidebar-width`.
  - **5. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors; `pnpm --filter @reachinternational/web build` passed with all 35/35 routes compiled cleanly (code 0).
- **Page Feedback: /operations?tab=entry — Blue Inner Toggle Background with White Text & Grey/Off-Black Inactive Period Display (`CustomTimePicker.tsx`, `TimeInput.tsx`) (2026-09-05)**:
  - **1. Vibrant Blue Active Inner Pill (`apps/web/components/ui/CustomTimePicker.tsx`)**: Replaced the white active sliding pill (`bg-white dark:bg-neutral-800`) with a vibrant primary blue pill (`bg-sky-600 dark:bg-sky-500 shadow-2xs`) in `<motion.div layoutId="ampm-active-..." />`. Switched active button typography from blue text to crisp, high-contrast white text (`text-white font-extrabold`). Styled inactive button typography in clean grey / off-black (`text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white font-bold`). Applied consistently across both horizontal and vertical toggle orientations.
  - **2. Cross-Platform Mobile App Synchronization (`apps/mobile/components/ui/TimeInput.tsx`)**: Synchronized React Native `TimeInput.tsx`: active button uses `backgroundColor: theme.colors.link` with text `color: '#ffffff'` and `fontWeight: '800'`. Inactive button uses `color: theme.colors.mute` and `backgroundColor: 'transparent'`.
  - **3. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors.
- **Page Feedback: /operations?tab=entry — Eliminate Duplicate Shift Timing Errors, Header Total Shift Time Display & Cross-Platform Error Box Optimization (`OperatorDashboard.tsx`, `CustomTimePicker.tsx`, `TimeInput.tsx`, `MeterLogModal.tsx`, `packages/utils/src/date.ts`) (2026-09-05)**:
  - **1. Shift Timing Header Total Duration Display (`apps/web/components/dashboard/OperatorDashboard.tsx`)**: Removed `operatingStats.errorMessage` from the top-right of the Shift Timing section header. When `operatingStats.durationMinutes > 0`, displays the total calculated shift time (`🌙 Overnight · {operatingStats.durationFormatted}` or `{operatingStats.durationFormatted}`) in bold monospace text. When times are empty or duration ≤ 0, renders nothing (`null`), leaving the header clean and uncluttered. Synchronized identical header duration in the Log Correction Modal (`editOperatingStats`).
  - **2. CustomTimePicker Error Redundancy Removal (`apps/web/components/ui/CustomTimePicker.tsx`, `apps/web/components/dashboard/OperatorDashboard.tsx`)**: Extended `CustomTimePickerProps` with `isInvalid?: boolean` and `hideErrorMessage?: boolean`. In `CustomTimePicker.tsx`, updated container border class to highlight with `border-rose-500` when `isInvalid` is true, without requiring an error message string that renders duplicate inline text. In Section B End Time picker, replaced `error={operatingStats.isFutureEnd ? ... : undefined}` with `isInvalid={operatingStats.isFutureEnd}`. In Edit Modal End Time picker, replaced `error={editOperatingStats.isFutureEnd ? ... : undefined}` with `isInvalid={editOperatingStats.isFutureEnd}`.
  - **3. Bottom Error Box Optimization (`apps/web/components/dashboard/OperatorDashboard.tsx`, `packages/utils/src/date.ts`)**: Retained the bottom error box as the ONLY location displaying shift timing errors. Optimized padding and spacing for mobile (≤640px) and desktop (≥1024px): `px-2.5 py-1.5 sm:px-3 sm:py-2`, `rounded-lg sm:rounded-xl`, `bg-rose-500/[0.08] dark:bg-rose-950/25 border border-rose-500/25 dark:border-rose-500/30`. Scaled typography and icon: `text-[11px] sm:text-xs font-semibold text-rose-600 dark:text-rose-400 leading-snug` with `<AnimatedAlertTriangle size={14} className="shrink-0 text-rose-500" />`. Streamlined shift timing error messages in `packages/utils/src/date.ts` (`"End time must be after start time."`, `"Shift cannot exceed 24 hours."`, `"Cannot log before shift end."`).
  - **4. Cross-Platform Mobile App Synchronization (`apps/mobile/components/ui/TimeInput.tsx`, `MeterLogModal.tsx`)**: Extended React Native `TimeInputProps` with `isInvalid?: boolean` and `hideErrorMessage?: boolean`. Highlighted container border in `theme.colors.error` when `isInvalid` is true. In `MeterLogModal.tsx`: Shift Timing header displays total duration (`{shiftStats.durationFormatted}`) in monospace badge when `shiftStats.durationMinutes > 0`. Replaced `error={...}` with `isInvalid={shiftStats.isFutureEnd}` on End Time `<TimeInput>`. Optimized bottom alert box with compact padding (`paddingHorizontal: 12, paddingVertical: 8`), icon, and concise error text.
  - **5. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors. Automated regression tests passed: `test_future_shift_validation.mjs` (9/9 passed, 100%), `test_breakdown_submission_and_linking.mjs` (all passed, 100%).
- **Page Feedback: /operations?tab=entry — Breakdown Timing Label Removal & Clean Top-Right Duration (`OperatorDashboard.tsx`, `MeterLogModal.tsx`) (2026-09-05)**:
  - **1. Removed Redundant "Breakdown Timing" Label (`apps/web/components/dashboard/OperatorDashboard.tsx`)**: Removed the redundant `<span className="text-[11px] font-bold ...">Breakdown Timing</span>` text and icon from the top header of the breakdown container. Retained strictly the total calculated time only (`{breakdownStats.durationFormatted}`, e.g., `3h:55min`) positioned cleanly on the top right (`flex justify-end`) in bold font-mono (`text-[11px] sm:text-xs font-mono font-bold text-rose-600 dark:text-rose-400`). Synchronized identical label removal in the Log Correction Modal (`editBreakdownStats`).
  - **2. Mobile App Cross-Platform Parity (`apps/mobile/components/work/MeterLogModal.tsx`)**: Removed the uppercase `Breakdown Timing` label in `MeterLogModal.tsx` and retained only the right-aligned duration (`{breakdownStats.durationFormatted}`).
  - **3. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=entry — TimePicker AM/PM Segmented Toggle Height, Width & Rounded-Full Edges Polish with Unified Padding (`CustomTimePicker.tsx`, `TimeInput.tsx`, `OperatorDashboard.tsx`) (2026-09-05)**:
  - **1. AM/PM Toggle Height, Width & Rounded-Full Edges (`apps/web/components/ui/CustomTimePicker.tsx`)**: Upgraded toggle container `div [Select AM or PM period]` to `h-7 sm:h-8` with `rounded-full` capsule curves and `border border-neutral-200/80 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900`. Sized `button "AM"` and `button "PM"` with `h-full`, `min-w-[28px] xs:min-w-[32px] sm:min-w-[36px]`, `px-2 sm:px-2.5`, `rounded-full`, and centered text. Enhanced typographic weight: inactive buttons use `font-bold text-neutral-500 hover:text-neutral-800 dark:hover:text-white`, and active buttons use `font-extrabold text-sky-600 dark:text-sky-400`. Updated active animated indicator (`motion.div layoutId="ampm-active-..."`) to `rounded-full`.
  - **2. Unified Input Shell Padding Across Start Time & End Time (`apps/web/components/ui/CustomTimePicker.tsx`, `apps/web/components/dashboard/OperatorDashboard.tsx`)**: Refined `<CustomTimePicker>` shell padding from `px-2 xs:px-2.5 sm:px-3` to `px-2.5 sm:px-3.5`, creating harmonious horizontal padding identical to `CustomDatePicker` (Log Date: `px-3 sm:px-3.5`) and the Overtime input (`px-2.5 sm:px-3.5`). Synchronized Section C breakdown timing grid and Log Correction Modal timing grids to `gap-2 sm:gap-3.5 items-start`.
  - **3. Cross-Platform Mobile App Synchronization (`apps/mobile/components/ui/TimeInput.tsx`)**: Synchronized React Native `TimeInput.tsx`: updated `periodContainer` to `height: 34`, `borderRadius: 17` (`rounded-full`), `padding: 2`. Sized `periodBtn` to `height: '100%'`, `minWidth: 32`, `paddingHorizontal: 8`, `borderRadius: 15` (`rounded-full`). Elevated inactive button text weight to `fontWeight: '700'` and active to `'800'`. Aligned `unifiedShell` padding to `paddingHorizontal: 12` to match web `px-3.5`.
  - **4. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=entry — Breakdown Duration Top-Right Header Relocation & Error-Only Display (`OperatorDashboard.tsx`, `MeterLogModal.tsx`) (2026-09-05)**:
  - **1. Breakdown Duration Top-Right Card Header Placement (`apps/web/components/dashboard/OperatorDashboard.tsx`)**: Removed bulky duration banner (`.p-2 sm:p-2.5 rounded-lg border flex ...`) from below the breakdown time pickers that previously pushed down Section D Remarks, Save Draft, and Submit actions. Integrated clean card header inside the breakdown container: left side displays uppercase tracking label `<AnimatedAlertTriangle size={13} /> Breakdown Timing`, right side displays total calculated time only (`{breakdownStats.durationFormatted}`, e.g. `3h:55min`, `25min`) in bold monospace text (`text-[11px] sm:text-xs font-mono font-bold text-rose-600 dark:text-rose-400`). Fixed header row layout prevents vertical layout shifts or pushing down of below form components when duration recalculates.
  - **2. Error-Only Display Below Time Pickers (`apps/web/components/dashboard/OperatorDashboard.tsx`)**: Area below time pickers renders nothing when valid (`breakdownStats.isValid`). Only renders an alert banner when an error occurs (`breakdownStats && !breakdownStats.isValid`) showing `{breakdownStats.errorMessage || "Please verify breakdown start and end times."}`. Synchronized identical top-right duration and error-only display in the Log Correction Modal (`editBreakdownStats`).
  - **3. Cross-Platform Mobile App Synchronization (`apps/mobile/components/work/MeterLogModal.tsx`)**: Replaced the bottom duration alert box in `MeterLogModal.tsx` with a clean card container header: left side displays `BREAKDOWN TIMING` in uppercase, right side displays total duration only (`{breakdownStats.durationFormatted}`). Positioned breakdown start and end `<TimeInput>` pickers in a side-by-side row. Configured area below time inputs to display only when `breakdownStats && !breakdownStats.isValid`, leaving space clean when valid and preventing modal scrolling.
  - **4. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors. Automated regression suites passed: `test_breakdown_submission_and_linking.mjs` (100%), `test_future_shift_validation.mjs` (9/9 passed).
- **Page Feedback: /operations?tab=entry — Unified TimePicker Redesign, Polished Horizontal AM/PM Segmented Toggle & Mobile/Desktop UX Standards (`CustomTimePicker.tsx`, `TimeInput.tsx`) (2026-09-05)**:
  - **1. Unified Input Container Architecture (`apps/web/components/ui/CustomTimePicker.tsx`)**: Replaced fragmented 3-box design with a single cohesive container shell (`min-h-[38px] sm:min-h-[42px] h-9.5 sm:h-[42px] px-2 xs:px-2.5 sm:px-3 rounded-xl border border-[var(--color-hairline)] bg-[var(--color-canvas)] shadow-2xs`). Integrated full focus ring (`focus-within:ring-2 focus-within:ring-sky-500/20 focus-within:border-sky-500`) and error highlight (`border-rose-500 focus-within:ring-rose-500/20`). Added container click handler directing focus to hours or minutes automatically.
  - **2. Polished Horizontal AM/PM Segmented Toggle (`apps/web/components/ui/CustomTimePicker.tsx`)**: Switched default toggle orientation to horizontal (`toggleOrientation = "horizontal"`). Designed sleek, high-contrast segmented pill (`bg-neutral-100 dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800 rounded-lg p-0.5`). Sized buttons with comfortable padding: `px-1.5 xs:px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] xs:text-[10.5px] sm:text-xs font-bold`. Powered active tab transition via Framer Motion spring physics (`motion.div layoutId="ampm-active-..."`).
  - **3. Keyboard Ergonomics & Auto-Advance / Retreat (`apps/web/components/ui/CustomTimePicker.tsx`)**: Auto-advance: typing 2 valid digits in hours automatically advances focus to minutes. Auto-retreat: pressing Backspace on empty minutes returns focus to hours. Arrow keys: ArrowUp/ArrowDown on hours increments/decrements (1–12 wrap-around); on minutes increments/decrements by 5. Select-on-focus: clicking into either field selects existing digits for instant replacement.
  - **4. Mandatory Cross-Platform Mobile App Synchronization (`apps/mobile/components/ui/TimeInput.tsx`)**: Synchronized React Native `TimeInput` with the same unified container shell (`unifiedShell: height: 44, paddingHorizontal: 10, borderRadius: 8, borderWidth: 1`). Updated digital inputs to frameless centered numbers with tabular-nums. Switched AM/PM switcher to horizontal segmented pill with comfortable touch padding (`paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6`). Added auto-advance to minutes input on 2 digits entered via ref.
  - **5. Monorepo Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` passed with 0 errors; `pnpm --filter @reachinternational/mobile exec tsc --noEmit` passed with 0 errors; `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=entry — Mobile Single-Row Timepickers, Concise Mobile Errors/Toasts/Dialogs & Cross-Platform Mobile Standards Optimization (`CustomTimePicker.tsx`, `OperatorDashboard.tsx`, `CustomDatePicker.tsx`, `Toast.tsx`, `TimeInput.tsx`, `MeterLogModal.tsx`) (2026-09-05)**:
  - **1. Mobile Single-Row Start & End Timepickers (`CustomTimePicker.tsx`, `OperatorDashboard.tsx`)**: Calibrated digit input dimensions to `w-[38px] xs:w-10 sm:w-11 shrink-0`, `h-8.5 xs:h-9 sm:h-10`, and `px-0.5 sm:px-1` with centered monospace font (`text-xs sm:text-sm font-bold font-mono text-center`), completely eliminating digit clipping while shrinking container width to ~143px. Fitted compact vertical AM/PM toggle (`size="xs"`, `min-h-[24px] xs:min-h-[26px] sm:min-h-[30px] px-1.5 sm:px-2 text-[9px] xs:text-[10px] sm:text-[11px]`). Configured Section B Shift Timing (`grid grid-cols-2 xl:grid-cols-4 gap-2 sm:gap-3.5 items-start`) and Section C Breakdown Timing (`grid grid-cols-2 gap-2 sm:gap-3 items-start`) so Start Time and End Time render cleanly in a single row side-by-side on mobile viewports (down to 320px–412px). Synchronized Log Correction Modal timing fields to `grid grid-cols-2 gap-2 sm:gap-4 items-start`.
  - **2. Concise Mobile Error & Warning Typography (`OperatorDashboard.tsx`, `CustomTimePicker.tsx`, `CustomDatePicker.tsx`)**: Shortened inline timepicker error text (`"Hour required (1–12)"`, `"Hour must be 1–12"`, `"Minutes must be 0–60"`). Shortened meter validation alert (`"End meter cannot be less than start meter."`), shift sequencing warning (`"Start ({start}) cannot precede prev shift end ({prevEnd})."`), handover button text (`"Align to {time}"` on mobile), shift overlap message (`"Shift overlaps with existing log ({range}) on this machine."`), datepicker indicator (`7d window`), and breakdown duration card header (`Duration:`) with zero duplicate duration strings.
  - **3. Mobile Dialog & Toast Standards (`OperatorDashboard.tsx`, `Toast.tsx`)**: Refactored confirmation dialog stat tiles to `grid grid-cols-3 gap-1.5 sm:gap-2.5`, rendering Shift Date, Hour Meter reading, and Overtime in a single row on mobile and saving >100px of vertical space to eliminate mobile modal scrolling. Updated confirmation modal footer reassurance label to `Direct commit` and submit CTA to `Confirm & Submit` on mobile. Refactored `<Toast>` notification positioning and padding to mobile standards: `top-3 sm:top-4 left-3 right-3 sm:left-auto sm:right-4 gap-1.5 sm:gap-2`, compact card `p-2.5 sm:p-4 min-w-0 w-full sm:min-w-[320px] sm:max-w-[400px] shadow-lg`, icon `h-7 w-7 sm:h-8 sm:w-8`, title `text-xs sm:text-sm font-bold`, description `text-[11px] sm:text-xs leading-snug`.
  - **4. Cross-Platform Mobile Parity (`apps/mobile/components/ui/TimeInput.tsx`, `MeterLogModal.tsx`)**: Updated mobile `TimeInput.tsx`: `digitBox` to `minWidth: 38`, `paddingHorizontal: 4`, `height: 42`; `periodBtn` to `paddingHorizontal: 8`, `minWidth: 34`, `height: 36`. Shortened inline error messages (`"Hour required (1–12)"`, `"Minutes must be 0–60"`). Updated `MeterLogModal.tsx`: shortened handover align button to `"Align to {time}"` and condensed shift sequencing alert.
  - **5. Monorepo Quality Gate**: `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=entry — Shift End Time Validation & Future Logging Prevention Across Frontend, Backend & Database (`date.ts`, `hourMeter.ts`, `OperatorDashboard.tsx`, `CustomTimePicker.tsx`, `operators.ts`, `TimeInput.tsx`, `MeterLogModal.tsx`, `046_validate_shift_end_not_future.sql`) (2026-09-05)**:
  - **1. Shared Date & Validation Utilities (`packages/utils/src/date.ts`, `packages/validation/src/hourMeter.ts`)**: Added `disallowFutureEnd?: boolean` and `currentTimestamp?: number` options to `computeShiftTiming`, returning `isValid: false`, `isFutureEnd: true`, and `errorMessage: "Cannot log before shift end."` when shift end is in the future. Added helper `isShiftEndInFuture`. Added `.refine()` to `CreateHourLogSchema` checking `end_datetime <= Date.now() + 60 * 1000`.
  - **2. Web Operations Entry & Correction Modal (`OperatorDashboard.tsx`, `CustomTimePicker.tsx`)**: Added 30-second interval ticker for live validation without re-typing. Bound `error={operatingStats.isFutureEnd ? "Cannot log before shift end." : undefined}` to Section B End Time picker, and bound `editOperatingStats.isFutureEnd` to Edit Modal End Time picker. Highlighted inputs with `border-rose-500` and single-line error badge in `<CustomTimePicker>`. Blocked submit triggers with toast notification.
  - **3. Server Action Backend Guards & DB Error Formatting (`apps/web/app/actions/operators.ts`)**: Added synchronous pre-validation guards in `submitOperatorHourLogAction` and `updateOperatorHourLogAction` checking `isShiftEndInFuture` with 1-minute grace margin. Formatted Postgres constraint/trigger error strings matching `"Cannot log before shift end"`.
  - **4. Cross-Platform Mobile Parity (`apps/mobile/components/ui/TimeInput.tsx`, `MeterLogModal.tsx`)**: Added `currentTick` ticker to `MeterLogModal.tsx`, passed `disallowFutureEnd: true`, bound error to End Time `<TimeInput>`, styled `border-rose-500` on error in `TimeInput.tsx`, and blocked mobile submission if shift end is in the future.
  - **5. Database Triggers & Atomic RPC (`supabase/migrations/046_validate_shift_end_not_future.sql`)**: Updated `check_machine_hour_log_shift_overlap()` trigger function and `submit_operator_hour_log_atomic()` RPC to reject logs where `end_datetime > NOW() + INTERVAL '1 minute'` with error `'Cannot log before shift end.'` (`ERRCODE = '23514'`).
  - **6. Automated Verification**: `node supabase/tests/test_future_shift_validation.mjs` (9/9 passed); `node supabase/tests/test_breakdown_submission_and_linking.mjs` (all passed); `tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=entry — Single-Row Start/End Time Layout & Vertical AM/PM Toggle Optimization (`SegmentedToggle.tsx`, `CustomTimePicker.tsx`, `OperatorDashboard.tsx`, `EditProfileModal.tsx`, `TimeInput.tsx`) (2026-09-05)**:
  - **1. Vertical AM/PM Toggle Architecture (`SegmentedToggle.tsx`, `CustomTimePicker.tsx`)**: Added first-class `orientation?: "horizontal" | "vertical"` to `<SegmentedToggle>` with vertical active pill Framer Motion spring physics. Configured `<CustomTimePicker>` to use vertical AM/PM orientation by default (`toggleOrientation = "vertical"`), matching input height (`h-8.5 xs:h-9 sm:h-10`) with compact width (`w-7 sm:w-8.5`), reclaiming over **40px of horizontal space per picker** (~80px per row). Bounded hours and minutes inputs to `flex-1 min-w-[34px] max-w-[48px]` with centered font-mono typography, eliminating digit clipping across all viewports.
  - **2. Single-Row Start Time & End Time Layout (`OperatorDashboard.tsx`, `EditProfileModal.tsx`)**: Refactored Section B Shift Timing to `grid grid-cols-2 xl:grid-cols-4 gap-2 sm:gap-3.5 items-start` with responsive order classes (`order-1` for Log Date, `order-2 sm:order-3 xl:order-2` for Start Time, `order-3 sm:order-4 xl:order-3` for End Time, `order-4 sm:order-2 xl:order-4` for Overtime), placing Start Time and End Time in a single row side-by-side on mobile (<640px) and tablet (640px–1279px), and 4-column row on desktop (≥1280px). Synchronized Breakdown timing and modal edit grids to 2 columns on mobile.
  - **3. Cross-Platform Mobile Parity (`apps/mobile/components/ui/TimeInput.tsx`)**: Updated React Native `periodContainer` to `flexDirection: 'column'`, `width: 34`, `height: 42`, with stacked AM and PM buttons, reclaiming ~40px per input and ensuring side-by-side time input pairs in `MeterLogModal.tsx` and `EditProfileModal.tsx` fit on all mobile screens without horizontal clipping.
  - **4. Monorepo Quality Gate**: `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=entry — Streamlined Confirmation Modal Layout, Spacing Optimization & Proportional Space Filling (`dialog.tsx`, `Modal.tsx`, `OperatorDashboard.tsx`) (2026-09-05)**:
  - **1. Header & Footer Vertical Space Reclamation (`dialog.tsx`, `Modal.tsx`, `OperatorDashboard.tsx`)**: Streamlined `DialogHeader` default padding from `p-6` to `px-5 py-3.5 sm:px-6 sm:py-4`, `DialogFooter` to `px-5 py-3 sm:px-6 sm:py-3.5`, and close button to `top-3.5 right-4 sm:top-4 sm:right-5`. Added `headerClassName`, `bodyClassName`, and `footerClassName` to `Modal.tsx` and tightened default body padding to `p-4 sm:p-5`. Removed redundant nested `pt-2` from confirmation modal footer in `OperatorDashboard.tsx`, reclaiming >50px of vertical space.
  - **2. Modal Width & Container Proportions (`OperatorDashboard.tsx`)**: Expanded modal width to `className="sm:max-w-[620px]"` to provide ample horizontal breathing room for operational metrics without awkward line breaks.
  - **3. Internal Content Re-architecture & Proportional Space Filling (`OperatorDashboard.tsx`)**:
    - *Deployment & Machine Context*: Divided 2-column card with Machine Model, Code badge, bold sky monospace Serial Number on left, and Client Name with Site Location and MapPin on right.
    - *Operational Shift & Metering*: Full-width Shift Window bar (`{startTime} → {endTime}`, Day/Overnight pill badge, duration + work hours) paired with a 3-quadrant balanced stat strip (Shift Date, Hour Meter reading progression with `+Xh Run` badge, Overtime badge).
    - *Machine Health Status*: High-contrast, clean status card with pulsing status dot (`🟢 Normal Operation · No Machine Breakdown` or structured rose card for Breakdowns).
    - *Remarks & Reassurance Footer*: Formatted quote block for remarks and `<ShieldCheck /> Direct database commit` reassurance badge on footer.
  - **4. Quality Gate Verification**: `pnpm --filter @reachinternational/web exec tsc --noEmit` and `pnpm -r exec tsc --noEmit` passed with 0 errors across all packages.
- **Bug Fix: /operations?tab=entry — Fix Machine Check Constraint Violation (machines_status_check), Resilient Atomic Breakdown Logging & Unified Error Handling (`actions/operators.ts`, `044_add_breakdown_time_tracking.sql`, `045_fix_breakdown_status_check_and_atomic_rpc.sql`, `MeterLogModal.tsx`, `test_breakdown_submission_and_linking.mjs`) (2026-09-05)**:
  - **1. Root Cause Fix for `machines_status_check` Constraint**: Fixed bug where breakdown hour logs failed with `code: '23514'` due to attempting to set `machines.status = 'under_maintenance'` (violating `machines_status_check`). Disentangled rental `status` (`'rented'`) from physical condition: updated `submit_operator_hour_log_atomic`, `submitOperatorHourLogAction`, and `updateOperatorHourLogAction` to set `health_status = 'breakdown'` (or `'active'`) and `current_operator_id = operator_id`, leaving `status` untouched.
  - **2. Resilient Error Formatting & PostgREST Conflict Shield (`actions/operators.ts`)**: Added `formatOperatorDatabaseError` mapping raw PostgreSQL codes (`23514`, `23505`, `23503`, `42501`, `P0001`) into user-friendly messages. Kept function internal without `export` to strictly conform to Next.js `"use server"` async Server Action requirements. Added automatic fallback to direct PostgreSQL table mutation whenever the RPC throws check constraint errors (`23514`) or candidate overload errors (`PGRST203`).
  - **3. Operator, Site & Machine Linking Hardening**: Ensured `clientId` and `location` automatically infer from machine assignment when omitted by client. Added `operatorId` parameter support for administrative overrides.
  - **4. Mobile App Cross-Platform Parity (`apps/mobile/components/work/MeterLogModal.tsx`)**: Updated mobile modal to update `health_status: isBreakdown ? 'breakdown' : 'active'` and `current_operator_id`, and added human-friendly error mapping for constraint violations and shift overlaps.
  - **5. Database Migrations (`044`, `045`)**: Corrected `044_add_breakdown_time_tracking.sql` and created `045_fix_breakdown_status_check_and_atomic_rpc.sql`.
  - **6. Verification & Automated Tests**: Passed `node supabase/tests/test_breakdown_submission_and_linking.mjs` with full test assertions on live database; passed monorepo TypeScript checks on web and mobile with 0 errors.
- **Page Feedback: /operations?tab=entry — Seamless Sidebar Expand/Shrink & Responsive Grid Optimization (`SegmentedToggle.tsx`, `CustomTimePicker.tsx`, `OperatorDashboard.tsx`, `TimeInput.tsx`) (2026-09-05)**:
  - **1. Reusable SegmentedToggle XS Size Variant (`SegmentedToggle.tsx`)**: Added `"xs"` size variant with compact padding and `cn()` class merging.
  - **2. Compact CustomTimePicker Architecture (`CustomTimePicker.tsx`)**: Switched AM/PM toggle to `size="xs"` (saving >100px), enforced `min-w-[44px] max-w-[72px] flex-1` on inputs with centered monospace formatting, completely resolving `0| : 0|` digit clipping.
  - **3. Fluid 3-Tier Shift Timing Grid (`OperatorDashboard.tsx`)**: Refactored Section B grid to `grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-2.5 sm:gap-3.5 items-start` with `min-w-0` on each column, allowing fluid adaptation during sidebar expand/shrink animations on 1366px screens, tablet 2-column pairing, and mobile 1-column stacking.
  - **4. Breakdown Section & Modal Resiliency (`OperatorDashboard.tsx`)**: Added `min-w-0` to breakdown timepickers, `min-w-0 flex-wrap gap-2` to breakdown duration badge, and `min-w-0` to correction modal timepicker grid.
  - **5. Mobile Parity (`apps/mobile/components/ui/TimeInput.tsx`)**: Set `minWidth: 44` and `paddingHorizontal: 6` on `digitBox` for touch clearance and narrow mobile screen robustness.
  - **6. Monorepo Quality Gate**: `pnpm -r exec tsc --noEmit` passed across all 9 packages with 0 errors.
- **Page Feedback: /operations?tab=history — Streamlined Operator Log History Table, Machine Serial Number Display, Two-Line Timestamps, Below-Reading Hour Meter & Synchronized PDF/Excel Reports (`packages/utils`, `OperatorDashboard.tsx`, `PrintableOperatorLogsModal.tsx`, `operator-logs-export.ts`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-03)**:
  - **1. Clock Icon Removal from Every Row (Feedback #1)**: Removed `<Clock>` icon from table rows in desktop data table, mobile cards, and column header in `OperatorDashboard.tsx`; removed `<Clock>` from mobile card specs in `apps/mobile/app/(app)/operations.tsx`.
  - **2. Two-Line Entry Timestamp (Feedback #2 & #3)**: Implemented `splitExactTimestamp` in `packages/utils/src/date.ts`. Top line shows Time (`06:15:24 PM` in bold font-mono), bottom line shows Date (`02-09-2026` in font-mono mute). Removed relative time `({formatTimeAgo})`.
...
- **Page Feedback: /operations?tab=history — Exact Operator Log Entry Timestamps Across Desktop Table, Mobile Cards & Cross-Platform Mobile Stack (`packages/utils`, `OperatorDashboard.tsx`, `OperationsClient.tsx`, `operator-logs-export.ts`, `PrintableOperatorLogsModal.tsx`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-03)**:
  - **1. Shared Exact Timestamp Formatter (`packages/utils/src/date.ts`)**:
    - Built canonical `formatExactTimestamp(dateInput, includeSeconds = true)` producing localized 12-hour AM/PM timestamps: `"DD-MM-YYYY, hh:mm:ss A"` (e.g. `"02-09-2026, 04:35:18 PM"`), or `"DD-MM-YYYY, hh:mm A"` when `includeSeconds = false`.
...
- **Page Feedback: /machines — Dynamic Multi-Personnel Table Cell Display & Scalable Font Sizing (`MachineListClient.tsx`, `MobileMachineCard.tsx`, `apps/mobile/app/(app)/machines.tsx`) (2026-09-02)**:
  - **1. Dynamic Up-to-3 Person Names Display (Feedback #1)**:
    - Updated supervisor and operator table columns in `<MachineListClient>` (`<EnterpriseTable>`) via dedicated `<PersonnelCell>` helper to display up to 3 person names directly in the table cell when multiple staff are assigned.
  ...
  - **2. Dynamic Typography Sizing Scaling**:
    ...
    - Automatically decreases font size as person names increase to fit all names properly, and increases font size as person names decrease.
...
- **Page Feedback: /operations?tab=entry — Compact Overnight Indicator & Cross-Platform Responsive Grid Optimization (`OperatorDashboard.tsx`, `CustomTimePicker.tsx`) (2026-09-02)**:
  - **1. Compact Overnight Indicator (Feedback #1)**: Removed bulky outer padding (`px-2.5 py-1`), border, background, and shadow from the `🌙 Overnight · [duration]` indicator in `OperatorDashboard.tsx`. Rendered as a sleek inline monospace text element (`text-[10px] sm:text-[11px] font-bold text-indigo-600 dark:text-indigo-400 font-mono inline-flex items-center gap-1`) that never pushes down lower form fields.
  - **2. Multi-Tier Responsive Grid Optimization (Feedback #2)**: Optimized Shift Timing fields (Log Date, Start Time, End Time, Overtime) to a fluid multi-tier grid (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5`). Updated `CustomTimePicker.tsx` inputs to `min-w-0 flex-1` with `shrink-0` colon and compact padding for flawless mobile responsiveness down to 320px screens.
  - **3. Monorepo Quality Gate Verification**: `pnpm turbo run typecheck` passed across **9/9 packages successful (0 errors)** in 19.767s.
- **Page Feedback: /operations?tab=entry — SegmentedToggle Reuse for AM/PM & Streamlined Handover Context Banner (`CustomTimePicker.tsx`, `OperatorDashboard.tsx`, `MeterLogModal.tsx`, `apps/mobile`) (2026-09-02)**:
  - **1. Reused Canonical `<SegmentedToggle>` for AM/PM (Feedback #1)**: Replaced the previous ad-hoc button container with the monorepo's canonical `<SegmentedToggle<"AM" | "PM">>` component in `CustomTimePicker.tsx` with dynamic `disabled` propagation and unique `useId()` layout prefix for smooth Framer Motion spring physics.
  - **2. Removed Redundant "Last shift ended" Container (Feedback #2)**: Removed the redundant nested `.flex` container displaying `"Last shift ended: [date], [time]"` in `OperatorDashboard.tsx` to streamline the shift entry banner.
  - **3. Full Date & Time in Handover Banner (Feedback #3)**: Retained the primary Handover element in `OperatorDashboard.tsx` and enhanced it to display both formatted date and time (`Handover from {machineTimeline.formattedEndDate}, {machineTimeline.formattedEndTime}`).
  - **4. Cross-Platform Mobile Parity (`apps/mobile`)**: Synchronized mobile `apps/mobile/components/work/MeterLogModal.tsx` machine timeline banner to display unified `"Handover from [date], [time]"` with clock icon.
  - **5. Monorepo Quality Gate Verification**: `pnpm turbo run typecheck` passed across **9/9 packages successful (0 errors)** in 39.985s.
- **Page Feedback: /operations?tab=entry — Leading Zero Normalization (010=10, 030=30), Time Sanitization & Operations Hub Query Optimization (`CustomTimePicker.tsx`, `TimeInput.tsx`, `packages/utils/src/date.ts`, `actions/operators.ts`, `queries/operators.ts`, `OperatorDashboard.tsx`) (2026-09-02)**:
...
- **Page Feedback: /machines/[id]/edit — Fixed Machine Personnel Assignments & MultiUserSelect Candidate Prioritization Across Frontend & Backend (`public.machines`, `MultiUserSelect.tsx`, `queries/machines.ts`, `machine-edit-client.tsx`, `MachineModal.tsx`) (2026-09-02)**:
  - **1. Database Multi-Assignment Normalization**: Cleaned up the assignment arrays in Supabase PostgreSQL database `dhbbgfzbyatzvqafnsqp` for machine `e956380b-d995-42e1-9b1e-f99a443f4587` (`RI-MC-0001`), setting its supervisor to single assigned supervisor `Monu Verma` (`ba8e6b2f-6ec3-4059-83e5-9d1cd4b344fd`) and operator to single assigned operator `Deepak Patel` (`18eb76dd-7fc8-40d6-a6ea-7f4869f1028b`) across both single-ID columns and array columns.
  - **2. Backend DAL Hydration Normalization (`queries/machines.ts`)**: In `hydrateMachinesPersonnel`, ensured `supervisor_ids` and `operator_ids` are mapped strictly from valid hydrated user objects (`cleanSupIds` and `cleanOpIds`).
  ...
  - **4. Monorepo Quality Gate Verification**: `pnpm turbo run typecheck` passed with **9/9 packages successful (0 errors)** in 15.097s; Next.js production build compiled **35/35 routes cleanly with code 0**.
- **Page Feedback: /operations?tab=entry — Complete Radial Clock Time Picker Removal & Manual Time Input Replacement Across Web and Mobile (`CustomTimePicker.tsx`, `TimeInput.tsx`, `TimePicker.tsx`, `DateTimePicker.tsx`, `OperatorDashboard.tsx`, `EditProfileModal.tsx`, `CreateTaskModal.tsx`, `MeterLogModal.tsx`, `apps/mobile`) (2026-09-02)**:
  - **1. Complete Radial Clock UI Removal**: Removed all SVG radial clock dials, pointer angle trigonometry calculations, dragging event listeners, and complex popover dialogs from `apps/web/components/ui/CustomTimePicker.tsx`.
  - **2. Manual Time Input Architecture**: Built `<TimeInput>` (aliased as `<CustomTimePicker>` / `<TimePicker>` for 100% monorepo backward compatibility):
  ...
  - **3. Cross-Platform Mobile App Synchronization (`apps/mobile`)**: Created native React Native `<TimeInput>` in `apps/mobile/components/ui/TimeInput.tsx` with min 44px touch targets. Synchronized `MeterLogModal.tsx` (Start & End Time), `EditProfileModal.tsx` (Shift Start & End Time), and `CreateTaskModal.tsx` (Due Time).
  - **4. Form Submission & Shift Workflow Verification**: Verified shift timing computation (`computeShiftTiming`), normal shifts (06:00 AM - 02:00 PM), handover shift sequencing, overnight shifts (08:00 PM - 06:00 AM), breakdown logging, and strict form submission blocking when invalid.
  - **5. Monorepo Quality Gate Verification**: `pnpm turbo run typecheck` passed across **9/9 packages successful (0 errors)** in 55.9s; Next.js production build compiled **35/35 routes cleanly with code 0**.
- **Page Feedback: /machines/[id] — Multiple Supervisors and Operators Assignment Support for 24h Fleet Coverage Across Monorepo Stack (`043_support_multiple_supervisors_and_operators.sql`, `@reachinternational/types`, `@reachinternational/validation`, `MultiUserSelect.tsx`, `actions/machines.ts`, `queries/machines.ts`, `queries/operators.ts`, `machine-client-view.tsx`, `machine-edit-client.tsx`, `MachineModal.tsx`, `MachineListClient.tsx`, `MobileMachineCard.tsx`, `machines-export.ts`, `PrintableMachineDirectoryModal.tsx`, `apps/mobile`) (2026-09-02)**:
  - **1. 24h Fleet Multi-Personnel Assignment**: Implemented multiple supervisor (`supervisor_ids UUID[]`) and multiple operator (`operator_ids UUID[]`) assignment support per machine to power 24-hour equipment operations across multiple 8-hour shifts.
  - **2. Live PostgreSQL Migration `043`**: Added GIN indexes (`idx_machines_supervisor_ids`, `idx_machines_operator_ids`), sync triggers maintaining `current_supervisor_id` & `current_operator_id` backward compatibility, updated supervisor operational restriction triggers, and updated RLS policies on Supabase database `dhbbgfzbyatzvqafnsqp`.
...
- **Page Feedback: /machines — Edit Profile Modal Streamlining, Clock UI Time Pickers, Section Consolidation & Address Simplification (`EditProfileModal.tsx`, `UserProfileDropdown.tsx`, `MobileBottomNav.tsx`, `apps/mobile`) (2026-09-02)**:
  - **1. Modal Title Standardization (Feedback #1)**: Changed title from `"Edit My Profile & Shift Details"` to `"Edit Profile"` in `EditProfileModal.tsx` and synchronized triggers across web and mobile.
  - **2. Direct Shift Start & End Time with Precision Clock UI (Feedback #2)**: Replaced preset select dropdown and custom text toggle with direct **Shift Start Time** and **Shift End Time** inputs connected to `<CustomTimePicker>` (SVG radial clock dial UI with instant hour, minute, AM/PM picking, and drag interaction).
  ...
  - **7. Removed Street Address Container (Feedback #7)**: Removed Street Address field completely; Section 3 strictly captures normalized location (**State / UT**, **District**, and **City / Town**).
...
- **Page Feedback: /machines — UserProfileDropdown Dynamic Profile Card Redesign, Full Detailed Info, Icon-Only Edit & Clean UI (`UserProfileDropdown.tsx`, `MobileBottomNav.tsx`) (2026-09-02)**:
  - **1. Removed Redundant Admin Links (Feedback #1)**: Removed `.space-y-0.5` links container (`User Management`, `Audit Security Logs`, `Platform Settings`) from the profile card popover.
  - **2. Slightly Bigger Dynamic View Card with Full User Info (Feedbacks #2 & #3)**: Expanded popover card width to `330px` with dynamic viewport-bounded positioning. Formatted email, mobile phone, shift schedule, residential/city/district/state address, masked Aadhaar (`XXXX-XXXX-1234`), and driving licence in high-contrast Geist micro-cards.
  - **3. Icon-Only Edit Action Button (Feedback #4)**: Replaced full text button with a top-right icon-only edit trigger button (`<Edit size={13} />`) in the profile header.
...
- **Page Feedback: /machines — Complete FilterToolbar Architecture Redesign from Scratch with CSS Grid & Hardware-Accelerated Animation (`FilterToolbar.tsx`) (2026-09-01)**:
  - **1. Architecture Redesign from Scratch (Feedback #1)**:
...
- **Page Feedback: /machines?tab=inventory — User Profile Details, Self-Service Edits, Hierarchical Approval Workflow, and Mobile Bottom Nav Cleanup (`042_add_shift_address_and_profile_change_requests.sql`, `profile_change_requests`, `MobileBottomNav.tsx`, `EditProfileModal.tsx`, `ProfileChangeRequestsSection.tsx`, `profile.tsx`, `users.tsx`, `apps/mobile`) (2026-09-01)**:
  - **1. Mobile Bottom Nav Cleanup**: Removed `"Quick Management"` paragraph and links from `<MobileBottomNav>`. Added profile summary card and self-service edit profile trigger.
  - **2. Full User Profile Details**: Added `shift_time` and `address` fields to `public.users` database table. Updated sidebar user profile dropdown (`UserProfileDropdown.tsx`), user detail sheets (`UserDetailSheet.tsx`), mobile profile screen (`profile.tsx`), and creation/edit forms (`UserCreateModal.tsx`, `UserEditModal.tsx`, `CreateUserModal.tsx`, `UserDetailModal.tsx`) to show shift timing, mobile, street address, city, state, masked Aadhaar, and driving licence.
  - **3. Self-Service Profile Edits & Approval Hierarchy Workflow**: Created `public.profile_change_requests` table with RLS and trigger (`042_add_shift_address_and_profile_change_requests.sql`). Implemented `apps/web/app/actions/profile.ts` and `EditProfileModal.tsx`:
    - `super_admin`: Direct instant database updates.
  ...
  - **4. Dedicated Profile Change Requests Review Section**: Created `<ProfileChangeRequestsSection>` on `/users` and mobile `apps/mobile/app/(app)/users.tsx` completely distinct from pending registrations (`status = 'pending'`), with side-by-side diffs, individual Approve/Reject, and batch Accept All / Reject All actions.
  - **5. Monorepo Quality Gate Verification**: `pnpm turbo run typecheck` passed (9/9 packages, 0 errors) and Next.js production build succeeded (35/35 routes compiled).
...
- **Page Feedback: /machines?tab=inventory — Machine Modal Header Cleanup & "Locked" Badge Removal (`MachineModal.tsx`, `AddMachineModal.tsx`) (2026-09-01)**:
  - **1. Removed Locked Badge (Feedback #1)**:
    ...
    - Cleaned mobile `apps/mobile/components/machines/AddMachineModal.tsx` input labels.
  - **2. Renamed Section 1 Header to "MACHINE INFO" (Feedback #2)**:
    - Changed Section 1 heading `<h4>` in `apps/web/components/machines/MachineModal.tsx` to `"MACHINE INFO"`.
...
- **Page Feedback: /machines?tab=inventory — Supervisor Machine Operational Status & Assignment Management Across Monorepo Stack (`041_allow_supervisor_machine_operational_updates.sql`, `matrix.ts`, `actions/machines.ts`, `MachineListClient.tsx`, `MachineModal.tsx`, `MobileMachineCard.tsx`, `apps/mobile/machines.tsx`, `AddMachineModal.tsx`) (2026-09-01)**:
  - **1. Fixed More Actions Menu for Supervisors (Feedback #1)**:
    - On `/machines` in `MachineListClient.tsx`, resolved the empty popover menu for supervisors by configuring `RowActionsMenu` to render authorized actions:
  ...
  - **2. Scope of Supervisor Updates (Feedback #1)**:
...
- **Page Feedback: /machines/[id] — Hours Meter Logs Streamlining, Overtime Removal & Collapsible Single-Row Filtering (`machine-client-view.tsx`) (2026-09-01)**:
...
- **Page Feedback: /machines/[id]/edit — Manager-or-Above Machine & Supervisor Management Across Full Stack, Title Standardization & Subtitle Removal (`040_restrict_machine_management_to_managers_and_admins.sql`, `roles.ts`, `matrix.ts`, `actions/machines.ts`, `actions/machine-import.ts`, `machine-edit-client.tsx`, `machines/[id]/edit/page.tsx`, `machines/[id]/page.tsx`, `MachineListClient.tsx`, `MachineModal.tsx`, `apps/mobile/machines.tsx`, `MachineDetailModal.tsx`) (2026-09-01)**:
  ...
  - **7. Monorepo Quality Gate & Verification**:
    ...
    - `pnpm --filter @reachinternational/web build` compiled all **35/35 Next.js App Router routes cleanly with code 0**.
- **Page Feedback: /machines/[id] — Full Filtering, Sorting, Quick Stats, Pagination & Mobile Toggle Optimization (`machine-client-view.tsx`, `SegmentedToggle.tsx`, `Table.tsx`) (2026-09-01)**:
...
- **Page Feedback: /machines/[id] — Mobile SegmentedToggle Optimization, Header Health Status Badge & Icon-Only Copy Buttons (`SegmentedToggle.tsx`, `machine-client-view.tsx`) (2026-09-01)**:
...
- **Page Feedback: /machines/[id] — UI Clean-Up, Icon Removal, Label Standardization & 3-Tier Mobile Responsiveness (`machine-client-view.tsx`, `Skeleton.tsx`, `MachineDetailModal.tsx`) (2026-09-01)**:
  ...
  - **10. Verification**:
    ...
    - `pnpm --filter @reachinternational/web build` compiled all **35/35 Next.js App Router routes cleanly with code 0**.
- **Page Feedback: /machines/[id] — Streamlined Hour Meter Running History UI & Table Columns (`machine-client-view.tsx`) (2026-09-01)**:
  - **1. Subtitle Removal (Feedback #1)**: Removed description paragraph text (`"Operator Daily Logbook with Shift Timings & Running Hours"`) from the Hour Meter Running History header card.
  - **2. Refresh Button Removal (Feedback #2)**: Removed the redundant `"Refresh"` button from the Hour Meter Running History card header.
  - **3. "+ Add Meter Log" Button Removal (Feedback #3)**: Removed the `"+ Add Meter Log"` button and navigation link from the card header.
  - **4. Redundant "Client Company" Column Removal (Feedback #4)**:
    - Removed `<TableHead>Client Company</TableHead>` from the desktop table header and corresponding `<TableCell>` from table body rows, eliminating repetitive client company column clutter on the dedicated machine detail view.
    - Cleaned mobile card view to eliminate redundant `Client: ...` strip.
  - **5. Verification**: `pnpm turbo run typecheck` passed with **9/9 packages successful (0 errors)** in 2m4.4s.
- **Global Action Button Optimization & Duplicate Submission Prevention (`Button.tsx` (web & mobile), `MobileBottomNav.tsx`, `UserProfileDropdown.tsx`, `PublicNavbar.tsx`, `OperationsClient.tsx`, `OperatorDashboard.tsx`, `ComplaintStatusModal.tsx`, `RentalReturnModal.tsx`, `SiteMovementModal.tsx`, `CreateTaskModal.tsx`) (2026-09-01)**:
  ...
  - **5. Verification**:
    ...
    - `pnpm --filter @reachinternational/web build` compiled all **35/35 Next.js App Router routes cleanly with code 0**.
- **Page Feedback: /machines/[id]/edit — 3-Tier Viewport Responsiveness Optimization (`machine-edit-client.tsx`, `page.tsx`, `loading.tsx`) (2026-09-01)**:
  ...
  - **6. Verification**:
    ...
    - `pnpm --filter @reachinternational/web build` compiled all **35/35 Next.js App Router routes cleanly with code 0**.
- **Bug Fix: /operations?tab=logs — Fixed Operator Assignment Error & Atomic Database Query Optimization (`039_assign_machine_operator_atomic.sql`, `actions/operators.ts`, `queries/operators.ts`, `OperationsClient.tsx`) (2026-09-01)**:
  - **1. Root Cause Resolution**: Resolved `Could not find the table 'public.machine_assignments' in the schema cache` runtime error when clicking "Confirm & Assign Operator" on `/operations?tab=logs` by removing obsolete queries/inserts to dropped `machine_assignments` table in favor of direct tracking in `public.machines.current_operator_id`.
...
- **Page Feedback: /machines — Assigned Client Visibility in Machine Directory Table & Full Query Optimization (`MachineListClient.tsx`, `queries/machines.ts`, `machines-export.ts`, `PrintableMachineDirectoryModal.tsx`, `MachineRow.tsx`, `apps/mobile/app/(app)/machines.tsx`) (2026-09-01)**:
...
- **Bug Fix: /machines/[id]/edit — Fixed Stuck "Saving Machine Details..." Loading State & Navigation Transition (`machine-edit-client.tsx`, `MachineModal.tsx`, `actions/machines.ts`, `machine-client-view.tsx`) (2026-09-01)**:
...
- **Page Feedback: /machines/[id] — High-Contrast, Smooth Animated Segmented Toggle Tab Navbar (`machine-client-view.tsx`) (2026-09-01)**:
  - **1. Visibility & Stacking Context**: Fixed z-index stacking hierarchy (`z-0` on active pill, `relative z-10` on content), preventing the active indicator from rendering behind the track container.
  - **2. Clean & Smooth Toggle**: High-contrast elevated track container with pure white active pill, sky blue active accent, dark mode compatibility, and fluid spring layout transition.
  - **3. Responsive Optimization**: 2-column equal-width toggle switch on mobile, inline segmented pill toggle on desktop.
  - **4. Verification**: Monorepo typecheck passed with 9/9 packages successful (0 errors).
- **Page Feedback: /operations?tab=entry — Remove ❌ Emojis & Shorten Machine Timeline Banner for Mobile Viewports (`OperatorDashboard.tsx`, `MeterLogModal.tsx`) (2026-09-01)**:
...
- **Page Feedback: /operations?tab=entry — Mobile Optimization, Responsive Segmented Switcher & Single-Row Start/End Meter Inputs (`OperatorDashboard.tsx`) (2026-09-01)**:
  - **1. Subnav Tab Switcher Optimization (Feedback #1 & #3)**: Replaced loose horizontal button row in the header card with a tactile, full-width 2-column segmented toggle on mobile (`grid grid-cols-2 gap-1.5 w-full p-1 sm:p-1.5 rounded-xl bg-[var(--color-hairline-soft-surface)]/80 dark:bg-neutral-900/60 border border-[var(--color-hairline)] shadow-2xs`) adapting to `sm:inline-flex sm:w-auto` on desktop, with `min-h-[42px]` touch targets, centered icon + label, and live log count badge.
  - **2. Start & End Meter in Single Row (Feedback #2)**: Configured Starting Hour Meter and Ending Hour Meter to always render side-by-side in a single row on both mobile and desktop (`grid grid-cols-2 gap-2.5 sm:gap-4`) with font-mono typography, `min-h-[42px]` touch targets, and clipboard paste validation with `HmrSchema`.
  - **3. Shift Timing & Modal Responsive Grids (Feedback #3)**: Enhanced Shift Timing into responsive grid (`grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5`) aligning Log Date (full width on mobile), Start & End Times (side-by-side in a single row on mobile), and Overtime. Updated Confirmation Modal stats grid to `grid-cols-2 sm:grid-cols-4` to prevent squishing on mobile viewports.
  - **4. Quality Verification**: Monorepo typecheck passed with **9/9 packages successful (0 errors)** in 42.9s.
- **Page Feedback: /operations?tab=logs — Remove Subtitle, Remove Notes Container & Restrict Operator Dropdown to Active Operators Only (`OperationsClient.tsx`, `UserSelect.tsx`, `queries/operators.ts`, `queries/machines.ts`, `mobile/AddMachineModal.tsx`) (2026-09-01)**:
...
- **Page Feedback: /machines/[id] — Mobile Icon-Only Action Buttons, Pixel-Perfect Alignment & Responsive Toggle-Type Segmented Tab Navbar (`machine-client-view.tsx`, `Button.tsx`) (2026-09-01)**:
...
- **Page Feedback: /machines/[id]/edit — Icon Removal, Subtitle Streamlining & Rented Client Selection Optimization (`machine-edit-client.tsx`, `MachineModal.tsx`, `page.tsx`, `queries/clients.ts`) (2026-09-01)**:
  - **1. Dynamic Client Selection on "Rented" Status (Feedback #1)**: Searchable `<ClientSelect>` dropdown displaying all active clients with client code and location chips when status is "Rented". Optimized parallel data fetch using `Promise.all` and lightweight column projections in `getClientOptions`.
  - **2. Subtitle Paragraph Removal (Feedback #2)**: Removed description paragraph from the header card on `/machines/[id]/edit`.
  - **3. Helper Span Removal (Feedback #3)**: Removed `<span>ℹ️ Select the client account currently renting this machine.</span>` helper text from the client select input.
  - **4. Save Button Icon Removal (Feedback #4)**: Removed `<AnimatedCheck>` icon from "Save Machine Details" submit button.
  ...
  - **8. Modal UI Streamlining (`MachineModal.tsx`)**: Removed section header icons for unified clean styling.
  - **9. Quality Verification**: Monorepo typecheck passed with **9/9 packages successful (0 errors)** in 2m41s.
- **Page Feedback: /machines/[id] — Machine Model-Serial No Title, Full Linked CRM Client Data & Lazy-Loaded Hour Meter Running History (`machine-client-view.tsx`, `machines.ts`, `database.ts`, `MachineDetailModal.tsx`, `mobile/machines.tsx`, `page.tsx`) (2026-09-01)**:
  - **1. Title Format Standardization**: Standardized primary header `<h1>` on `/machines/[id]` to `[Model] - [Serial No]` (e.g. `50B-9 - TY50B-99214`) with copyable Machine ID badge and manufacturer metadata in the sub-header row.
  - **2. Linked CRM Client Entity**: Joined and displayed complete CRM client entity from `public.clients` (Company name, Client code `CLI-XXXX`, contact person, mobile with `tel:`, email, city/district/state, GSTIN with copy, PAN with copy, site address with copy, conditional billing address with copy, active rental contract details, and 1-click Call/WhatsApp/Maps touch actions).
...
- **Bug Fix: /machines — Resolve React Runtime Crash in EnterpriseTable (`EnterpriseTable.tsx`) (2026-09-01)**:
  - **1. Root Cause**:
    - In `apps/web/components/ui/EnterpriseTable.tsx` (bulk actions bar), `typeof Icon === "function"` was used to check if the action icon was a component or an element.
    - Since `lucide-react` icons and `forwardRef` components are React objects (`{ $$typeof: Symbol(react.forward_ref), render: ... }`), `typeof Icon === "function"` evaluated to `false`.
    - This routed into the `: ( Icon )` branch, causing React to attempt rendering the raw forwardRef object as a child in JSX, immediately triggering `Objects are not valid as a React child (found: object with keys {$$typeof, render})`.
  - **2. Fix**:
    - Replaced with `isValidElement(Icon) ? Icon : <AnimateIcon icon={Icon as any} animation="bounce" size={13} />` with `isValidElement` imported from `"react"`, properly handling both pre-instantiated JSX elements and React component/forwardRef types.
  - **3. Verification**:
    - `pnpm turbo run typecheck --force` passed with **9/9 packages successful (0 errors)** in 33.5s.
- **Page Feedback: /machines/[id]/edit — Dynamic Client Selection on Rented Status & High-Performance Parallel Queries (`037_add_client_id_to_machines.sql`, `database.ts`, `machine.ts`, `queries/clients.ts`, `queries/machines.ts`, `actions/machines.ts`, `machine-edit-client.tsx`, `MachineModal.tsx`, `MobileMachineCard.tsx`, `MachineRow.tsx`, `AddMachineModal.tsx`, `mobile/machines.tsx`, `MachineDetailModal.tsx`) (2026-09-01)**:
  ...
  - **7. Verification**:
    ...
    - `pnpm --filter @reachinternational/web build` compiled all **35/35 Next.js App Router routes cleanly with code 0**.
- **Page Feedback: /machines — Multi-Format Export (Excel .xlsx, CSV .csv, PDF .pdf/Print) & Duplicate CSV Button Removal (`machines-export.ts`, `PrintableMachineDirectoryModal.tsx`, `EnterpriseTable.tsx`, `MachineListClient.tsx`) (2026-09-01)**:
