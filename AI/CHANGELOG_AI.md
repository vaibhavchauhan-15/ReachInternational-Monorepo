- **Phase 14 Planning: Mobile Offline Sync & Network Resilience (`.planning/phases/14-mobile-offline-sync-network-resilience/14-01-PLAN.md`, `14-02-PLAN.md`, `14-VALIDATION.md`) (2026-09-09)**:
...
- **Monorepo-Wide Canonical `<Pagination />` Integration Across All Tables & Tabs (`apps/web/*`) (2026-09-09)**:
...
- **Architecture & Agent Protocol: Multi-Agent Engineering Team Rule (`.agents/rules/multi_agent_engineering_team.md`, `AI/RULES/MULTI-AGENT-ENGINEERING-TEAM.md`, `AGENTS.md`, `.agents/rules/mandatory_rules_reading_and_enforcement.md`) (2026-09-09)**:
...
- **Users Performance & Server-Side Pagination: /users?tab=all — 10 Users/Page Server Pagination, Parallelized Relational Hydration, Cached Aggregates, and Global Metrics Sync (`apps/web/lib/queries/users.ts`, `apps/web/app/(app)/users/page.tsx`, `apps/web/app/(app)/users/users-client.tsx`) (2026-09-09)**:
...
- **Users Performance: /users?tab=all — Server-Side Pagination, Debounced Search, Indexed Sorting & Mobile Parity (`supabase/migrations/057_add_users_pagination_search_indexes.sql`, `apps/web/lib/queries/users.ts`, `apps/web/app/actions/users.ts`, `apps/web/app/(app)/users/page.tsx`, `apps/web/app/(app)/users/users-client.tsx`, `apps/mobile/app/(app)/users.tsx`) (2026-09-08)**:
  - **1. Database Composite Indexes (`057_add_users_pagination_search_indexes.sql`)**: Created GIN indexes `pg_trgm` on `full_name`, `email`, `phone`, `city`, `district`, `state`, `aadhaar_number`, `license_number` and B-tree indexes for `created_at`, `role`, `status`, `state`.
  - **2. Server-Side Paginated Query (`apps/web/lib/queries/users.ts`)**: Built `getUserList()` with exact count, range pagination (`(page-1)*pageSize`), extensive SQL `ilike` text search, filters, and batched hydration. Added `exportUsersFilteredAction` in `apps/web/app/actions/users.ts`.
  - **3. URL-Driven State (`apps/web/app/(app)/users/page.tsx`)**: Wired search params (`page`, `search`, `role`, `status`, `kyc`, `state`, `dateRange`, `sort`) into Next.js App Router for bookmarkable filters.
  - **4. Client-Side UX & Fixed Pagination (`apps/web/app/(app)/users/users-client.tsx`)**: Replaced client-side in-memory filter logic with URL routing, added 300ms debounced search, implemented unpaginated Server Action Excel/CSV export, and integrated canonical `<Pagination>` component.
  - **5. Mobile App Synchronization (`apps/mobile/app/(app)/users.tsx`)**: Replaced client-side local search loop with server-side `.range()` pagination (20 rows/page) and an appended "Load More" button following strict UI/Mobile Sync Rules.
- **Operations Performance: /operations?tab=logs — Server-Side Pagination, Debounced Search, Indexed Sorting & Mobile Parity (`supabase/migrations/058_optimize_hour_logs_pagination.sql`, `apps/web/lib/queries/operators.ts`, `apps/web/app/(app)/operations/page.tsx`, `apps/web/components/operations/OperationsClient.tsx`, `apps/mobile/app/(app)/operations.tsx`) (2026-09-09)**:
  - **1. Database Composite Indexes (`058_optimize_hour_logs_pagination.sql`)**: Added `idx_machine_hour_logs_date_created (log_date DESC, created_at DESC, id DESC)` and `idx_machine_hour_logs_client_date (client_id, log_date DESC)` to optimize default global sorting, pagination, and client-scoped filtering without full-table scans.
  - **2. Resilient Paginated Query (`apps/web/lib/queries/operators.ts`)**: Built `getOperationsLogsPage()` with exact count, minimal projections, 3-tier resilient fallbacks, and server-side global search across machines, operators, remarks, and locations via pre-resolved ID matching.
  - **3. Slim Tab Loader (`apps/web/lib/queries/operators.ts`)**: Streamlined `getOperationsHubData()` so the logs tab loads only a 10-row slice and lightweight machine options.
  - **4. URL-Driven State (`apps/web/app/(app)/operations/page.tsx`)**: Extended Next.js query parameter passthrough (`page`, `view`, `machine`, `client`, `operator`, `month`, `start`, `end`, `search`, `sort`) supporting bookmarkable and shareable filter views.
  - **5. Client-Side UX & Fixed Pagination (`apps/web/components/operations/OperationsClient.tsx`)**: Integrated 300ms debounced search, clear input button, `isPending` transitions, true row offset calculation, and canonical `<Pagination>` (fixed 10 items/page, no page-size selector) on desktop table and mobile touch card views.
  - **6. Mobile App Synchronization (`apps/mobile/app/(app)/operations.tsx`)**: Replaced 100-log client filtering with server-side `.range()` pagination (10 rows/page), 400ms debounced search, and native touch pagination controls with min 44px touch targets.
  - **7. Quality Gates**: Zero TypeScript errors (`pnpm -r exec tsc --noEmit`) and 75/75 operator test suite assertions passed.
- **User Feature: /clients?tab=all — Server-Side Pagination & Mobile App Sync (`apps/web/lib/queries/clients.ts`, `apps/web/app/(app)/clients/page.tsx`, `apps/web/components/clients/ClientsClient.tsx`, `apps/mobile/app/(app)/clients.tsx`) (2026-09-08)**:
  - **1. Server-Side Pagination**: Implemented `getPaginatedClients` utilizing Supabase `.range()` and `.or()` ilike search to fetch clients 10 at a time, drastically reducing DB load and memory footprint.
  - **2. Extracted Metrics Query**: Created a lightweight `getClientMetrics` query to accurately compute total, active, inactive, and locations covered across the entire dataset without fetching heavy row data.
  - **3. URL-Driven State**: Replaced `useMemo` full-dataset filtering with URL `searchParams` state (`page`, `search`, `status`) to match the pagination pattern used in the `/machines` directory.
  - **4. Debounced Search**: Implemented 300ms debouncing for the text search input to update URL params seamlessly.
  - **5. Canonical Pagination UI**: Integrated the standard `<Pagination />` component at the bottom of the Web desktop table and Web mobile touch cards view, with a subtle `useTransition` loading state.
  - **6. Mobile App Synchronization**: Added pagination state (`page`, `pageSize`) and Prev/Next UI to `apps/mobile/app/(app)/clients.tsx` to precisely mirror the Web App's 10-item limit and pagination experience, fully adhering to the mandatory web-to-mobile sync rule.
- **User Feature & Page Feedback: /users?tab=all — Name (15 chars) & Email (20 chars) Truncation, Col 4 Clean Supervisor Badges/Chips, Portal 3-Dot Multi-Supervisor Checkbox List & Multi-Supervisor Architecture (`supabase/migrations/056_add_user_supervisor_ids.sql`, `packages/types/src/database.ts`, `apps/web/lib/queries/users.ts`, `apps/web/app/actions/users.ts`, `apps/web/app/(app)/users/UserRow.tsx`, `apps/web/app/(app)/users/MobileUserCard.tsx`, `apps/web/app/(app)/users/UserDetailSheet.tsx`, `apps/web/app/(app)/users/users-client.tsx`, `apps/mobile/app/(app)/users.tsx`, `apps/mobile/components/users/UserDetailModal.tsx`) (2026-09-08)**:
  ...
  - **3. 3-Dot Dropdown Multi-Supervisor Checkbox Selector**: Relocated supervisor assignment inside the fixed portal dropdown menu. Features a scrollable checkbox list with min 44px touch targets, a "No Supervisor" clearing option, and optimistic state synchronization for instant feedback.
  ...
  - **7. User Detail Sheet & Responsive Card Parity**:
    - `UserDetailSheet.tsx`: Updated info row to render all assigned supervisors with email chips, and added interactive multi-supervisor checkbox selector in management controls.
...
- **Page Feedback: /users?tab=all — Table Header Labels Simplified: "User Account" → "Name" & "Role & Access Level" → "Role" (`apps/web/app/(app)/users/users-client.tsx`) (2026-09-08)**:
...
- **Page Feedback: /users?tab=all — Whole Table Row & Card Clickable to View User Details, Removal of Redundant Eye Icon & Clean 3-Dot Action Column (`apps/web/app/(app)/users/UserRow.tsx`, `apps/web/app/(app)/users/users-client.tsx`) (2026-09-08)**:
...
- **Page Feedback: /operations?tab=history — Change Button Name to "Export / Print" & Standalone "Export Excel" Removal Confirmation (`apps/web/components/dashboard/OperatorDashboard.tsx`, `apps/web/components/operations/OperationsClient.tsx`, `apps/web/components/dashboard/PrintableOperatorLogsModal.tsx`) (2026-09-08)**:
...
- **Page Feedback: /operations?tab=entry — Remove Duplicate "Export Excel" Button, Strip Breakdown Outer Padding, and Relocate Total Min Badge to Top Left across Web & Mobile (`apps/web/components/dashboard/OperatorDashboard.tsx`, `apps/mobile/components/work/MeterLogModal.tsx`) (2026-09-08)**:
...
- **Page Feedback: /operations?tab=entry — Thin Overtime Input (2-Digit), Reduced Time Picker Width & Increased AM/PM Toggle Width across Web & Mobile (`apps/web/components/ui/CustomTimePicker.tsx`, `apps/web/components/dashboard/OperatorDashboard.tsx`, `apps/mobile/components/ui/TimeInput.tsx`) (2026-09-08)**:
...
- **Page Feedback: /operations?tab=entry — Conditional Remarks on Breakdown, Duplicate Red Star Removal, Check Icon Removal & Mobile Synchronization (`apps/web/components/dashboard/OperatorDashboard.tsx`, `apps/mobile/components/work/MeterLogModal.tsx`) (2026-09-08)**:
...
- **Page Feedback: /operations?tab=entry — Separate AM/PM Toggle Shifted to the Right of Time Picker & Multi-Phone Mobile Optimization across Web & Mobile (`apps/web/components/ui/CustomTimePicker.tsx`, `apps/mobile/components/ui/TimeInput.tsx`) (2026-09-08)**:
